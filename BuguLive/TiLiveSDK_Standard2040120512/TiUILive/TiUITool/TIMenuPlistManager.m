...
//               {
//           NSLog(ASLocalizedString(@"沙盒中存在 %@"),path);
//                 plistDictionary = [TIUITool getJsonDataForPath:filePatch];
//                }
//                else
//                {
//            NSLog(ASLocalizedString(@"沙盒中不存在 %@"),path);
////     每次启动默认加载本地配置
...
//            }
           ...
           for (NSDictionary *dic in plstArr) {
               ...
               if ([path isEqualToString:@"stickers"]||[path isEqualToString:@"gifts"]||[path isEqualToString:@"watermarks"]||[path isEqualToString:@"masks"]||[path isEqualToString:@"greenscreens"]) {
