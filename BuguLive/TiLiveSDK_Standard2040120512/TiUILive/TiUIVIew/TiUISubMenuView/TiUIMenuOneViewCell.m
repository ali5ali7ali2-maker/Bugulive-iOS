...
- (void)setMode:(TIMenuMode *)mode{
    if (mode) {
       _mode = mode;
    [self.totalSwitch setTitle:[NSString stringWithFormat:ASLocalizedString(@"%@:关"),mode.name] withImage:[UIImage imageNamed:@"btn_close"] withTextColor:TI_Color_Default_Text_Black forState:UIControlStateNormal];
    [self.totalSwitch setTitle:[NSString stringWithFormat:ASLocalizedString(@"%@:开"),mode.name] withImage:[UIImage imageNamed:@"btn_open"] withTextColor:TI_Color_Default_Background_Pink forState:UIControlStateSelected];
