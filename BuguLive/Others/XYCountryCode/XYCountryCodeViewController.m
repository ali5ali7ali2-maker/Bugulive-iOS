...
- (void)setup{
    ...
    if(_type == XYCountryCodeShowTypeNone){
        ...
        searchBar.placeholder = ASLocalizedString(@"输入关键字");
    ...
    UIBarButtonItem *rightBarButtonItem  =  [[UIBarButtonItem alloc] initWithTitle:ASLocalizedString(@"确定") style:UIBarButtonItemStylePlain target:self action:@selector(leftBarButtonItemSelect:)];
...
-(UIView *)pickerBGView{
    if (!_pickerBGView) {
        ...
        [_pickerBGDemoBT setTitle:ASLocalizedString(@"确定") forState:UIControlStateNormal];
        if (@available(iOS 13.0, *)) {
