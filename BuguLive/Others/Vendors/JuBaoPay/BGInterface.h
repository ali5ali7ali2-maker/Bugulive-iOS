...
@interface BGInterface : NSObject
/*
   @abstract    初始化接口
 ...
 */
+ (void)start:(UIViewController *)controller withParams:(BGParam *)params withType:(NSUInteger)type withDelegate:(id<FWInterfaceDelegate>)delegate;
...
/*
    ...
    推荐使用这种
*/
+ (void)start:(UIViewController *)controller withParams:(BGParam*)params withDelegate:(id<FWInterfaceDelegate>)delegate;
/*
 启动接口
 controller 视图控制器
 params 支付参数
 delegate 支付回调对象
 */
+ (void)start:(UIViewController *)controller withParams:(BGParam*)params withDelegate:(id<FWInterfaceDelegate>)delegate withView:(UIView*)view;
