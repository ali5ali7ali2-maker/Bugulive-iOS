...
@interface TCShowLiveMsgTableViewCell : UITableViewCell<MLEmojiLabelDelegate>
{
    ...
    UIImageView             *_msgBgImgView;  // 等级图标
    UIImageView             *_rankImgView;  // 等级图标
    NSString                *_type;         // 类型
    UIImageView *_vipView;
    UIImageView *_stealthView;//隐身
    UIImageView *_guardianView;//守护
//    UIImageView *_guardianBGView;//守护
    UIImageView *_nobleVipView;//贵族
    
}

@property (nonatomic, weak) id<TCShowLiveMsgTableViewCellDelegate>  delegate;
@property (nonatomic, strong) UILabel                          *msgLabel;              // 消息显示标签
