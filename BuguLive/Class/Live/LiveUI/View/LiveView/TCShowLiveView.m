...
- (instancetype)initWith:(id<FWShowLiveRoomAble>)liveItem liveController:(id<FWLiveControllerAble>)liveController
{
    ...
    {
        _liveItem = liveItem;
        self.is_voice = liveItem.is_voice;
        ...
        for (UIView *subView in self.superview) {
...
- (void)refreshLiveItem:(id<FWShowLiveRoomAble>)liveItem liveInfo:(CurrentLiveInfo *)liveInfo
{
    ...
    [_voicebottomView refreshLiveItem:liveItem liveInfo:liveInfo];
    if (self.currentLiveInfo.push_type.integerValue == 1) {
        _soundSetBt.hidden = !_isHost;
    }else{
        _soundSetBt.hidden = YES;
    }
    ...
    if ([_liveItem liveType] == FW_LIVE_TYPE_RELIVE)
    {
        ...
        {
            tmpY = kScreenH - kSendGiftContrainerHeight - 50 - CGRectGetHeight(_msgView.frame) - MG_BOTTOM_MARGIN;
        }
        else
        {
...
- (void)addOwnViews
{
    ...
    _topView.clickVipBlock = ^(BOOL isClick) {
        [weakSelf clickShowVipView:nil];
    };
    ...
    _bottomView.heartRect = CGRectMake(kScreenW-kDefaultMargin*2-kMyBtnWidth1*2+5, kScreenH-kDefaultMargin-kMyBtnWidth1+5 - MG_BOTTOM_MARGIN, kMyBtnWidth1-10, kMyBtnWidth1-10);
    [self addSubview:_bottomView];
    
    _voicebottomView = [[TCShowVoiceBottomView alloc] init];
    _voicebottomView.isHost = _isHost;
    _voicebottomView.delegate = self;
    _voicebottomView.heartRect = CGRectMake(kScreenW-kDefaultMargin*2-kMyBtnWidth1*2+5, kScreenH-kDefaultMargin-kMyBtnWidth1+5 - MG_BOTTOM_MARGIN, kMyBtnWidth1-10, kMyBtnWidth1-10);
    _voicebottomView.hidden = YES;
    ...
    _soundSetBt = [UIButton buttonWithType:UIButtonTypeCustom];
    [_soundSetBt setBackgroundImage:[UIImage imageNamed:@"live_sound_btn"] forState:UIControlStateNormal];
//    [_soundSetBt setImage:[UIImage imageNamed:@"live_sound_btn"] forState:UIControlStateNormal];
    //2020-1-3 修改音效位置
    _soundSetBt.frame = CGRectMake(kScreenW - kRealValue(37) - kRealValue(5), kScreenH - 65- 37-8, kRealValue(37), kRealValue(37));
    [_soundSetBt addTarget:self action:@selector(clickSound:) forControlEvents:UIControlEventTouchUpInside];
//    [_soundSetBt setBackgroundColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:0.4]];
    _soundSetBt.layer.masksToBounds = YES;
    _soundSetBt.layer.cornerRadius = kRealValue(44 / 2);
    [self addSubview:_soundSetBt];
    if (self.currentLiveInfo.push_type.integerValue == 1) {
        _soundSetBt.hidden = !_isHost;
    }else{
        _soundSetBt.hidden = YES;
    ...
    _livewWishView = [MGShowLiveWishView new];
    //4-18 心愿单显示
    _livewWishView.hidden = YES;
    if ([_liveItem liveType] == FW_LIVE_TYPE_HOST) {
        [_livewWishView requestModel:@""];
    }
    else
    {

        [_livewWishView requestModel:_roomIDStr];
    }
    _livewWishView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.4];
    [self addSubview:_livewWishView];
    
    _liveVipBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [_liveVipBtn addTarget:self action:@selector(clickShowVipView:) forControlEvents:UIControlEventTouchUpInside];
    [_liveVipBtn setImage:[UIImage imageNamed:ASLocalizedString(@"live_show_vipView")] forState:UIControlStateNormal];
    _liveVipBtn.hidden = [[GlobalVariables sharedInstance].appModel.open_noble isEqualToString:@"0"];
    _liveVipBtn.hidden = YES;
    [self addSubview:_liveVipBtn];
...
- (void)setIs_voice:(BOOL)is_voice
{
    _is_voice = is_voice;
    if(_is_voice == YES)
    {
...
- (void)addSomeListener
{
    ...
    _tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onTapBlank:)];
...
- (void)showCloseLiveBtn
{
    ...
    {
        FWWeakify(self)
        UIImage *closeImage = [UIImage imageNamedL:@"bm_daily_task"];
        
        _closeLiveBtn = [[MenuButton alloc] initWithTitle:nil icon:closeImage action:^(id<MenuAbleItem> menu) {
            FWStrongify(self)
...
-(void)clickSound:(UIButton *)sender{
    
    [self hiddenGiftView];
    
    if (_uiDelegate && [_uiDelegate respondsToSelector:@selector(showSoundSetView:)])
    {
        [_uiDelegate showSoundSetView:self];
    }
}
...
-(void)clickShowVipView:(UIButton *)sender{
    
    [self hiddenGiftView];
    
    if (_uiDelegate && [_uiDelegate respondsToSelector:@selector(showVipView:)])
    {
        [_uiDelegate showVipView:self];
    }
}
...
- (BMPopBaseView *)dailyTaskPop
{
    ...
    [_dailyTaskPop updateUIframeWithWidth:kScreenW - 41 * 2 andHeight:kRealValue(460) andTitleStr:@"" andmyEunmType:BMEachDaytask];
...
- (void)loadGiftView:(NSArray *)list
{
    ...
    // 礼物视图
    _giftView = [[GiftView alloc]initWithFrame:CGRectMake(0, kScreenH, kScreenW, _giftViewHeight)];
    _giftView.isVoice = self.is_voice;
    _giftView.isHost = self.isHost;
    _giftView.delegate = self;
    _giftView.hidden = YES;
    _giftView.isVoice = YES;
    _giftView.isHost = self.isHost;
    
    _giftView.clickShadowBlock = ^(BOOL click) {
...
- (void)relayoutFrameOfSubViews
{
    ...
    if(self.is_voice)
    {
        [_topView setFrameAndLayout:CGRectMake(0, kStatusBarHeight, 200, 50)];
        if(self.isRTL)
        {
            [_topView setFrameAndLayout:CGRectMake(kScreenW-200, kStatusBarHeight, 200, 50)];
        }
    }
    else
    {
        [_topView setFrameAndLayout:CGRectMake(0, kStatusBarHeight, rect.size.width, kLogoContainerViewHeight+66 + 56)];
        if(self.isRTL)
        {
            [_topView setFrameAndLayout:CGRectMake(kScreenW-rect.size.width, kStatusBarHeight, rect.size.width, kLogoContainerViewHeight+66 + 56)];
        }
    }
    ...
    if ([_liveItem liveType] == FW_LIVE_TYPE_RELIVE)
    {
        ...
        _bottomView.frame = CGRectMake(0, kScreenH-kMyBtnWidth1-kDefaultMargin - MG_BOTTOM_MARGIN, rect.size.width, kMyBtnWidth1);
        ...
        {
            tmpY = kScreenH - kSendGiftContrainerHeight - 50 - COMMENT_TABLEVIEW_HEIGHT - MG_BOTTOM_MARGIN;
        }
        else
        {
            tmpY = kScreenH - kSendGiftContrainerHeight - 10 - COMMENT_TABLEVIEW_HEIGHT - MG_BOTTOM_MARGIN;
        }
    ...
    else
    {
        ...
        {
            _bottomView.frame = CGRectMake(0, kScreenH-kMyBtnWidth1-kDefaultMargin-_goldFlowerViewHeiht - MG_BOTTOM_MARGIN, rect.size.width, kMyBtnWidth1);
            
            _liveInputView.frame = _bottomView.frame;
            
            CGFloat tmpY = kScreenH - kSendGiftContrainerHeight - 10 - COMMENT_TABLEVIEW_HEIGHT - _goldFlowerViewHeiht - MG_BOTTOM_MARGIN;
            _msgView.frame = CGRectMake(kDefaultMargin, tmpY, rect.size.width * 0.8, COMMENT_TABLEVIEW_HEIGHT);
        }
        else if (_guessSizeViewCanNotSee == NO && _guessSizeView)
        {
            _bottomView.frame = CGRectMake(0, kScreenH-kMyBtnWidth1-kDefaultMargin-kGuessSizeViewHeight - MG_BOTTOM_MARGIN, rect.size.width, kMyBtnWidth1);
            
            _liveInputView.frame = _bottomView.frame;
            
            CGFloat tmpY = kScreenH - kSendGiftContrainerHeight - 10 - COMMENT_TABLEVIEW_HEIGHT - kGuessSizeViewHeight - MG_BOTTOM_MARGIN;
            _msgView.frame = CGRectMake(kDefaultMargin, tmpY, rect.size.width * 0.8, COMMENT_TABLEVIEW_HEIGHT);
        }
        else
        {
            ...
            CGFloat tmpY = kScreenH - kSendGiftContrainerHeight - 10 - COMMENT_TABLEVIEW_HEIGHT - MG_BOTTOM_MARGIN;
            _msgView.frame = CGRectMake(kDefaultMargin, tmpY, rect.size.width * 0.8, COMMENT_TABLEVIEW_HEIGHT);
        }
    }
    _voicebottomView.frame = CGRectMake(0, kScreenH-kMyBtnWidth1-kDefaultMargin - MG_BOTTOM_MARGIN, rect.size.width, kMyBtnWidth1);

    _livewWishView.frame = CGRectMake(kDefaultMargin, _topView.bottom + kRealValue(5), kRealValue(120), kRealValue(40));
    _livewWishView.layer.masksToBounds = YES;
    _livewWishView.layer.cornerRadius = kRealValue(40 / 2);
    _liveInputView.height = KBOTTOM_HEIGHT;
    _liveInputView.y = kScreenH-KBOTTOM_HEIGHT;
//    -kDefaultMargin;
    if(self.is_voice)
    {
        _livewWishView.top -= 26;
        _livewWishView.right = SCREEN_WIDTH - 8;
    }
    else
    {
        _livewWishView.bottom = self.topView.bottom;

    }
    _liveVipBtn.frame = CGRectMake(kScreenW - kRealValue(10) - kRealValue(60), kRealValue(150) + kRealValue(15), kRealValue(60), kRealValue(60));
    
//    [_livewWishView relayoutFrameOfSubViews];
    [_bottomView relayoutFrameOfSubViews];
    [_liveInputView relayoutFrameOfSubViews];
    [_msgView relayoutFrameOfSubViews];
    
    _closeLiveBtn.frame = CGRectMake(kScreenW - kDefaultMargin - 50 - 5, CGRectGetMaxY(self.topView.accountLabel.frame)+1.5*kDefaultMargin+kStatusBarHeight, 50 , 42);
    _closeLiveBtn.bottom = self.topView.bottom;
//
//    [_closeLiveBtn mas_makeConstraints:^(MASConstraintMaker *make) {
////        make.width.equalTo(@(GourdViewWidth));
////        make.height.equalTo(@(GourdViewHeight));
////        make.right.equalTo(self);
//        make.centerX.equalTo(self.gourdView);
//        make.top.equalTo(self.gourdView.mas_bottom);
//    }];
...
- (void)sendMessage
{
    ...
    if (_liveInputView.barrageBtn.isSelected)
    { //弹幕消息
    ...
    { //普通消息F
        ...
        scmm.is_guardian = self.currentLiveInfo.is_guardian;
        scmm.guardianModel = [GlobalVariables sharedInstance].guardianModel;
        scmm.guardian_broadcast = [GlobalVariables sharedInstance].guardianModel.guardian_broadcast;
        scmm.guardian_remind = [GlobalVariables sharedInstance].guardianModel.guardian_remind;
        
        scmm.guardian_skin = [GlobalVariables sharedInstance].guardianModel.guardian_skin;
        scmm.guardian_gift = [GlobalVariables sharedInstance].guardianModel.guardian_gift;
        scmm.guardian_img = [GlobalVariables sharedInstance].guardianModel.guardian_img;
        scmm.guardian_kick = [GlobalVariables sharedInstance].guardianModel.guardian_kick;
        scmm.guardian_icon = [GlobalVariables sharedInstance].guardianModel.guardian_icon;
        
       
        
        
        scmm.is_vip = [NSString stringWithFormat:@"%ld",self.currentLiveInfo.is_vip];
...
{
    _currentDiamonds = [[IMAPlatform sharedInstance].host getDiamonds];
    if (_currentDiamonds < self.BuguLive.appModel.bullet_screen_diamond && !_isHost)
    {
        [self showRechargeAlert:ASLocalizedString(@"当前余额不足，充值才能继续发送弹幕，是否去充值？")];
    ...
    [self.httpsManager POSTWithParameters:mDict SuccessBlock:^(NSDictionary *responseJson) {
        ...
        {
            // 本地金额递减
            self.currentDiamonds -= self.BuguLive.appModel.bullet_screen_diamond;
...
- (void)senGift:(GiftView *)giftView AndGiftModel:(GiftModel *)giftModel
{
    ...
    else
    {
        ...
        if([BGUtils isBlankString:giftModel.num])
        {
        ...
        {
            [mDict setObject:giftModel.num forKey:@"num"];

        }        [mDict setObject:_roomIDStr forKey:@"room_id"];
        [mDict setObject:[NSString stringWithFormat:@"%ld",(long)giftModel.is_plus] forKey:@"is_plus"];
        
        if([GlobalVariables sharedInstance].giftSelecUserId == nil && self.is_voice == YES)
        {
            [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请选择一个用户")];
            return;
        }
        
        if([GlobalVariables sharedInstance].giftSelecUserId)
        {
            [mDict setObject:[GlobalVariables sharedInstance].giftSelecUserId forKey:@"to_user_id"];
        }
        FWWeakify(self)
        [self.httpsManager POSTWithParameters:mDict SuccessBlock:^(NSDictionary *responseJson) {
...
-(void)showGiftView:(NSString *)uid
{
    ...
    _bottomView.hidden = YES;
    _giftView.hidden = NO;
    _giftView.selectUid = uid;
    [_giftView show:self.superview];
    [_giftView.shadowViews addGestureRecognizer:_tapGesture];
    ...
    [UIView animateWithDuration:0.1 animations:^{
        ws.giftView.frame = CGRectMake(0, kScreenH-_giftViewHeight - MG_BOTTOM_MARGIN, _giftView.frame.size.width, _giftView.frame.size.height);
...
{
    if(bottomView == self.voicebottomView)
    {
        if(button.tag == EFunc_VOICE_GIFT)
        {
            [self showGift];
        }
        else if(button.tag == EFunc_VOICE_FACE)
        {
            if (_serveceDelegate && [_serveceDelegate respondsToSelector:@selector(clickFace:)])
            {
                [_serveceDelegate clickFace:self];
            }
        }
        else if(button.tag == EFunc_VOICE_MUSIC)
        {
            if (_serveceDelegate && [_serveceDelegate respondsToSelector:@selector(clickFace:)])
            {
                [_serveceDelegate clickMusic:self];
            }
        }
        else if(button.tag == EFunc_VOICE_CHART)
        {
        ...
        else if(button.tag == EFunc_VOICE_MORE)
        {
            if (_sdkDelegate && [_sdkDelegate respondsToSelector:@selector(clickVoiceMore:)])
            {
                [_sdkDelegate clickVoiceMore:self];
            }
        }
        else if(button.tag == EFunc_VOICE_CONNECT_MIKE)
        {
            if (_sdkDelegate && [_sdkDelegate respondsToSelector:@selector(clickVoiceMic:)])
            {
                [_sdkDelegate clickVoiceMic:self];
            }
        }
        else if(button.tag == EFunc_VOICE_INPUT)
        {
            _bankerViewCanSee = NO;
            [_liveInputView becomeFirstResponder];
            _liveInputView.hidden = NO;
        ...
        else if(button.tag == EFunc_VOICE_GAME)
        {
            
            VoiceMoreView *gameView = [[[NSBundle mainBundle]loadNibNamed:@"VoiceMoreView" owner:self options:nil]lastObject];
            gameView.user_role = self.currentLiveInfo.user_role;
            gameView.liveId = self.currentLiveInfo.room_id;
            gameView.frame = CGRectMake(0, self.height - 430, self.width, 430);
            gameView.clickWishListBlock = ^{
                [self.serveceDelegate clickWishView:self];
            };
            
            gameView.clickManagementBlock = ^{
                [self.serveceDelegate clickRoomManage:self];
            };
            [gameView show:self type:FDPopTypeBottom];
...
//        readView.diamonds = [NSString stringWithFormat:@"%@",responseJson[@"diamonds"]];
    ...
    else if (button.tag == EFunc_AUCTION && [self.BuguLive.appModel.open_pai_module integerValue] == 1)
    { // 竞拍
    ...
    else if (button.tag == EFunc_PK)
    {
        if (_serveceDelegate && [_serveceDelegate respondsToSelector:@selector(clickPkList:)])
        {
            [_serveceDelegate clickPkList:self];
        }
    }else if (button.tag == EFunc_SHOP)
    {
        if (_serveceDelegate && [_serveceDelegate respondsToSelector:@selector(clickShowShopView:)])
        {
            [_serveceDelegate clickShowShopView:self];
        }
    }
    else if (button.tag == EFunc_WISH){
        if (_serveceDelegate && [_serveceDelegate respondsToSelector:@selector(clickWishView:)])
        {
            [_serveceDelegate clickWishView:self];
        }
    }
    else if (button.tag == EFunc_CLOSE){
        if (_serveceDelegate && [_serveceDelegate respondsToSelector:@selector(clickCloseBtn:)])
        {
            [_serveceDelegate clickCloseBtn:self];
        }
    }
    ...
    else if (button.tag == EFunc_SWITCH_BANNER)//上庄下庄控制按钮
    {
        if (_goldFlowerView || _guessSizeView)
        {
            ...
            {
                NSString * str = _banker_status == 0 ? ASLocalizedString(@"是否开启上庄？"): ASLocalizedString(@"确定要移除该庄家？");
                
                FWWeakify(self)
                [FanweMessage alert:ASLocalizedString(@"提示")message:str destructiveAction:^{
        ...
        [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson) {
            ...
            if ([responseJson toInt:@"status"] == 1)
            {
                ...
                if (self.BuguLive.appModel.open_diamond_game_module == 1)
                {
                    ...
                    self.bankerView.myCoinLabel.text = [NSString stringWithFormat:ASLocalizedString(@"余额:%@"),coinStr];
                }
                else
                {
...
- (void)onTapBlank:(UITapGestureRecognizer *)tap
{
    if (tap.state == UIGestureRecognizerStateEnded)
    {
        ...
        //发送点亮消息
        if (isShowLight && !_isHost && _bottomView.canSendLightMsg && _bottomView.lightCount <= 7 && self.BuguLive.appModel.is_no_light != 1)
        {
...
- (void)displayGameWinWithGainModel:(GameGainModel *)gainModel
{
    ...
    [UIView animateWithDuration:1.0 delay:0 usingSpringWithDamping:0.95 initialSpringVelocity:10 options: UIViewAnimationOptionCurveLinear  animations:^{
        ...
        if (self.BuguLive.appModel.open_diamond_game_module == 1)
        {
            self.gameWinView.gameWinLab.text = [NSString stringWithFormat:@"%@%@",gainModel.gain,self.BuguLive.appModel.diamond_name];
        }
        else
        {
            self.gameWinView.gameWinLab.text = [NSString stringWithFormat:ASLocalizedString(@"%@游戏币"),gainModel.gain];
        }
...
- (void)exchangeViewDownWithExchangeCoinView:(ExchangeCoinView *)exchangeCoinView
{
    if (_exchangeView == exchangeCoinView)
    {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            self.exchangeView.coinLabel.text = ASLocalizedString(@"0游戏币");
...
- (void)obtainBankerCoin
{
    ...
    [self.httpsManager POSTWithParameters:mDict SuccessBlock:^(NSDictionary *responseJson) {
        ...
        if ([responseJson toInt:@"status"] == 1)
        {
            ...
            self.gameBankerView.bankerDesLabel.text = [NSString stringWithFormat:ASLocalizedString(@"剩余底金%@"),model.coin];
        }
...
- (void)clickMoreToolsView:(MoreToolsView *)moreToolsView andToolsModel:(ToolsModel *)model
{
    ...
    if ([model.title isEqual:ASLocalizedString(@"星店")])
    {
        [self clickStarShopButton];
    }
    else if ([model.title isEqual:ASLocalizedString(@"小店")])
    {
...
- (void)addGameDataWithDic:(NSDictionary *) responseJson
{
    ...
    if (dic && [dic isKindOfClass:[NSDictionary class]])
    {
        ...
        if (_banker_status == 2)
        {
            ...
            self.gameBankerView.bankerDesLabel.text = [NSString stringWithFormat:ASLocalizedString(@"剩余底金%@"),_liveGameModel.banker.coin];
        ...
        else if (_banker_status == 1)
        {
            ...
            self.gameBankerView.hidden = YES;
            _bankerView.leastCoinLabel.text = [NSString stringWithFormat:ASLocalizedString(@"底金:%@"),_liveGameModel.principal];
        }
        else
        {
            _bottomView.grabBankerBtn.hidden = YES;
...
- (void)changeBeginButtonWithCanBeginNext:(BOOL )canBeginNext
{
    ...
    else
    {
        ...
        _bottomView.beginGameBtn.icon = [UIImage imageNamedL:@"gm_game_wait"];
        [_bottomView.beginGameBtn setImage:[UIImage imageNamedL:@"gm_game_wait"]  forState:UIControlStateNormal];
        [_bottomView.beginGameBtn setImage:[UIImage imageNamedL:@"gm_game_wait"] forState:UIControlStateHighlighted];
    }
}
...
- (void)gameBankerMessageWithCustomMessageModel:(CustomMessageModel *)customMessageModel
{
    ...
    if (action == 1)
    {
        ...
        else
        {
            ...
            _bankerView.leastCoinLabel.text = [NSString stringWithFormat:ASLocalizedString(@"底金:%@"),principal];
            _bottomView.grabBankerBtn.hidden = NO;
        }
    }
    else if (action ==2)
...
- (void)loadBankerDataWithModel:(CustomMessageModel *)customMessageModel withHadLeft:(BOOL) hadLeft
{
    if (_banker_status == 2)
    {
        ...
        self.gameBankerView.bankerDesLabel.text = [NSString stringWithFormat:ASLocalizedString(@"剩余底金%@"),customMessageModel.banker.coin];
        [self changeGameBankerView];
    ...
    else if (_banker_status == 1)
    {
        ...
        if ([[customMessageModel.data allValues] containsObject:@"principal"]) {
            _bankerView.leastCoinLabel.text = [NSString stringWithFormat:ASLocalizedString(@"底金:%@"),[customMessageModel.data objectForKey:@"principal"]];
        }
    }
    else
    {
...
{
    NSString * str = _isArc ? ASLocalizedString(@"是否切换为手动发牌"): ASLocalizedString(@"是否切换为自动发牌");
...
- (void)updateCharmValue:(NSString *)charmValue userId:(NSString *)userId
{
    
}

- (void)showRedPacketWithCustomMessageModel:(CustomMessageModel *)customMessageModel
{
    
    RedPacketView *redView = [[RedPacketView alloc] initWithAvatarImage:customMessageModel.head_image nickname:customMessageModel.nick_name];
    [redView showInView:self];
    redView.customMessageModel = customMessageModel;
}
@end
