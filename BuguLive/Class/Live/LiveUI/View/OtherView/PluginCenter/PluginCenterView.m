...
{
//    _bgView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, self.size.width, self.size.height)];
////    _bgView.backgroundColor = RGB(213, 214, 216);
//    _bgView.image = [UIImage imageNamed:@"lr_plugin_bg"];
...
//    [_bgView addSubview:effectView];
    ...
    _headLab = [[UILabel alloc]initWithFrame:CGRectMake(14, 14, self.size.width-kDefaultMargin*2, pluginTitleHeight-14)];
    _headLab.text = ASLocalizedString(@"基础工具");
    [_headLab setFont:[UIFont systemFontOfSize:14]];
    _headLab.textColor = RGB(255, 255, 255);
    _headLab.textAlignment = NSTextAlignmentLeft;
    [self addSubview:_headLab];
    ...
    _toolsView = [[ToolsView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(_headLab.frame), self.size.width, height)];
...
//    [_bgView addSubview:secondLine];
    ...
    UILabel *secondHeadLab = [[UILabel alloc]initWithFrame:CGRectMake(14, _toolsView.height + _toolsView.frame.origin.y, self.size.width-kDefaultMargin*2, pluginTitleHeight-10)];
    secondHeadLab.text = ASLocalizedString(@"游戏与功能列表");
    secondHeadLab.font = [UIFont systemFontOfSize:14];
    secondHeadLab.textColor = RGB(255, 255, 255);
    secondHeadLab.textAlignment = NSTextAlignmentLeft;
    [self addSubview:secondHeadLab];
...
//    threeLine.backgroundColor = [kAppPluginSpaceColor colorWithAlphaComponent:0.5];
    ...
    layout.itemSize = CGSizeMake((self.size.width - 20 - 20 * 5) / 4 - 1, (self.size.width - 20 - 20 * 5) / 4 - 1);
    layout.sectionInset = UIEdgeInsetsMake(20, 20, 20, 20);
    layout.minimumLineSpacing = 20;
    layout.minimumInteritemSpacing = 20;
    _gameCollectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(10, secondHeadLab.height + secondHeadLab.frame.origin.y, self.size.width - 20, self.size.height-CGRectGetMaxY(secondHeadLab.frame)-kDefaultMargin) collectionViewLayout:layout];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if ([model.is_active isEqualToString:@"1"])
    {
        ...
        {
            cell.gamePlaying_L.text = ASLocalizedString(@"使用中");
        }
        else
        {
            cell.gamePlaying_L.text = ASLocalizedString(@"使用中");
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        [_httpManager POSTWithParameters:dict SuccessBlock:^(NSDictionary *responseJson) //通过该网络请求来判断功能插件的开启是否冲突
         {
             [[BGHUDHelper sharedInstance] stopLoading:hud];
