...
@interface BGTLiveBeautyView()<UIPickerViewDataSource,UIPickerViewDelegate,UIGestureRecognizerDelegate>
{
...
@implementation BGTLiveBeautyView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _beautyBtnNameArray = [NSArray arrayWithObjects:ASLocalizedString(@"关闭美颜"),
                               ASLocalizedString(@"普通美颜"),
                               ASLocalizedString(@"浪漫"),
                               ASLocalizedString(@"清新"),
                               ASLocalizedString(@"唯美"),
                               ASLocalizedString(@"粉嫩"),
                               ASLocalizedString(@"怀旧"),
                               ASLocalizedString(@"蓝调"),
                               ASLocalizedString(@"清凉"),
                               ASLocalizedString(@"日系"),
                               nil];
        
        if([GlobalVariables sharedInstance].openAgora)
        {
            _beautyBtnNameArray = [NSArray arrayWithObjects:ASLocalizedString(@"关闭美颜"),ASLocalizedString(@"普通美颜"),
                                   nil];;
        }
        ...
        _curEffectIdx = 1;
        // 修改美颜参数
        _filterParam1 = [self addSliderName:ASLocalizedString(@"美颜")From:0 To:100 Init:50];
...
- (void)onBtnClick:(id)sender
{
    ...
    {
        _beautyBgView.frame = CGRectMake(0, self.frame.size.height-(_beautyBtnContrianerViewHeight+80+40), self.frame.size.width, _beautyBtnContrianerViewHeight+80 + 40);
        [self layoutMyCommponent];
    }
    ...
    else if ([_currentBeautyBtnNameStr isEqualToString:ASLocalizedString(@"普通美颜")])
    {
        lookupFileName = ASLocalizedString(@"普通美颜");
    }
    else if ([_currentBeautyBtnNameStr isEqualToString:ASLocalizedString(@"浪漫")])
    {
        lookupFileName = @"langman.png";
    }
    else if ([_currentBeautyBtnNameStr isEqualToString:ASLocalizedString(@"清新")])
    {
        lookupFileName = @"qingxin.png";
    }
    else if ([_currentBeautyBtnNameStr isEqualToString:ASLocalizedString(@"唯美")])
    {
    ...
    else if ([_currentBeautyBtnNameStr isEqualToString:ASLocalizedString(@"怀旧")])
    {
        lookupFileName = @"huaijiu.png";
    }
    else if ([_currentBeautyBtnNameStr isEqualToString:ASLocalizedString(@"蓝调")])
    {
        lookupFileName = @"landiao.png";
    }
    else if ([_currentBeautyBtnNameStr isEqualToString:ASLocalizedString(@"清凉")])
    {
        lookupFileName = @"qingliang.png";
    }
    else if ([_currentBeautyBtnNameStr isEqualToString:ASLocalizedString(@"日系")])
    {
...
- (void)onSlider:(id)sender
{
    if (sender != _filterParam1 &&
        sender != _filterParam2 && sender != _filterParam3)
    {
        return;
    }
...
+ (UIImage*)KSYGPUImageNamed:(NSString*)name
{
    ...
    UIImage *imageFromKSYBundle = [UIImage imageWithContentsOfFile:[[[BGTLiveBeautyView KSYGPUResourceBundle] resourcePath] stringByAppendingPathComponent:name]];
