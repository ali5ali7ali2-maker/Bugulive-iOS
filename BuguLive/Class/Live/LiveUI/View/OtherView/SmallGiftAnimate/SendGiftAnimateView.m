...
- (void)setContent:(CustomMessageModel *)customMessageModel
{
    ...
    if(customMessageModel.type == 1)
    {
        ...
        if (customMessageModel.sender.is_noble_mysterious.intValue == 1) {
            _titleNameLabel.text = ASLocalizedString(@"神秘人") ;
            [_headImgView sd_setImageWithURL:nil forState:UIControlStateNormal placeholderImage:kDefaultNobleMysteriousHeadImg];
        }else{
        ...
        _numLabel.borderColor = RGB(255,193,10);
    }
...
//        _giftNameLabel.text = [NSString stringWithFormat:ASLocalizedString(@"参与%@次出价"),customMessageModel.pai_sort];
