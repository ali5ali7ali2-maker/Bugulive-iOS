...
//    self.redButtonSpaceH.constant = self.redButtonSpaceH.constant *kScreenH/480;
    ...
    self.luckNewButton.hidden = YES;
    self.myTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    self.myTableView.backgroundColor = kClearColor;
    self.myTableView.tableHeaderView = [[UIView alloc]initWithFrame:CGRectZero];
    [self.myTableView registerNib:[UINib nibWithNibName:@"SRedBagViewCell" bundle:nil] forCellReuseIdentifier:@"SRedBagViewCell"];
    [BGMJRefreshManager refresh:self.myTableView target:self headerRereshAction:@selector(loadRedBagNet) shouldHeaderBeginRefresh:NO footerRereshAction:nil];
    
    self.headImgView.layer.cornerRadius = 45 / 2;
    self.headImgView.layer.masksToBounds = YES;
    
    self.headImgView3.layer.cornerRadius = 68 / 2;
    self.headImgView3.layer.masksToBounds = YES;
    self.headImgView3.hidden = YES;
...
//    [self addGestureRecognizer:tap1];
    ...
    [self.headBackBroundView setImage:[UIImage imageNamedL:@"bg_room_red_BigHead"]];
    [self.luckNewButton setLocalizedString];
    [self.messageLabel setLocalizedString];
...
- (void)creatRedWithModel:(CustomMessageModel *)model
{
    ...
    [self.headImgView3 sd_setImageWithURL:[NSURL URLWithString:model.sender.head_image]];
    ...
    self.nameLabel2.text = [NSString stringWithFormat:ASLocalizedString(@"%@的红包"),model.sender.nick_name];
    self.nameLabel3.text = [NSString stringWithFormat:ASLocalizedString(@"%@的红包"),model.sender.nick_name];
}
...
//    self.headBackBroundView.hidden = YES;
    ...
    [self.headBackBroundView setImage:[UIImage imageNamed:@"bg_room_red_packet_head"]];
    ...
    self.headImgWidthConstraint.constant = 278;
    self.headImgHeightConstraint.constant = 404;
    
    self.nameLabel2.hidden = NO;
    self.luckButton.hidden = YES;//旧的隐藏掉
    self.smallHeadBackBroundView.hidden = NO;
    self.smallHeadView.hidden = NO;
    self.nameLabel2.hidden = NO;
    self.messageLabel2.hidden = NO;
    self.luckNewButton.hidden = NO;
...
- (IBAction)redButtonClick:(UIButton *)sender
{
    ...
    [dict setObject:@"surprise" forKey:@"ctl"];
    [dict setObject:@"requestGetSurprise" forKey:@"act"];
    [dict setObject:_user_prop_id forKey:@"user_prop_id"];
    
    [dict setObject:self.customMessageModel.dicData[@"surprise_id"] forKey:@"surprise_id"];
    [dict setObject:self.video_id forKey:@"video_id"];
    FWWeakify(self)
...
- (IBAction)buttonClick:(id)sender
{
    ...
    [self.headBackBroundView setImage:[UIImage imageNamed:@"bg_room_red_shouqi"]];
    ...
    self.headImgHeightConstraint.constant = 408;
    self.headImgWidthConstraint.constant = 279;
    self.headImgView3.hidden = NO;
