...
#import "MGLiveRechargeView.h"//充值界面
...
//接收到表情
- (void)recvEmoji:(CustomMessageModel *)model;
...
@protected
    ...
    // 礼物相关
    __weak BGIMMsgHandler       *_iMMsgHandler;             // IM处理单例
    ...
    //座驾
    CarAnimationPlayer             *_aCYCarView;
    NSMutableArray               *_aCYCarViewwArray;             // 座驾入视图队列
    BOOL                         _aCYCarViewwShowing;            // 座驾是否正在显示 高级别观众进入视图
    WardPopView *_wardPopView;
    WardOpenView *_wardOpenView;
...
// 直播UI层
@property (nonatomic, strong) BGLiveUIViewController    *liveUIViewController;
// 当前直播实体
...
// 中奖提示视图
@property (nonatomic, strong) WinTipView   *winTipView;
// 中奖提示视图队列
@property (nonatomic, strong) NSMutableArray            *winTipViewArray;
// 中奖正在显示开通提示
@property (nonatomic, assign) BOOL                      winTipViewShowing;
...
@property (nonatomic, strong) BGFinishLiveView            *finishLiveView;


@property(nonatomic, strong) MGGlobalVipView *globalView;//广播视图

@property(nonatomic, copy) void (^clickCloseBlock)(BOOL isReresh);
