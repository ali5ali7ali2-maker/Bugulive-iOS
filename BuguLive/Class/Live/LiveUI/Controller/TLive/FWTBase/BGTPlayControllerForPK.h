...
@class BGTPlayControllerForPK;

@protocol FWTPlayControllerForPKDelegate <NSObject>
@required

// 结束回播，用来判断是否还有下一段回播
- (void)stopReLive;
// 首帧回调
- (void)firstFrame:(BGTPlayControllerForPK*)playVC;
// 网络断连,且经多次重连抢救无效后退出app
- (void)exitPlayAndApp:(BGTPlayControllerForPK*)publishVC;
// 网络断连,重连
- (void)playAgain:(BGTPlayControllerForPK*)publishVC isHideLeaveTip:(BOOL)isHideLeaveTip;

@end


@interface BGTPlayControllerForPK : UIViewController
{
...
@property (nonatomic, weak) id<FWTPlayControllerForPKDelegate>delegate;
