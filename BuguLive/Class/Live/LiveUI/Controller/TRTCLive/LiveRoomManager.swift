...
class LiveRoomManager: NSObject {
    ...
    func isVideoMuted(forUser userId: String) -> Bool {
        guard let remoteUser = roomUserMap[userId] as? LiveRemoteUser else {
            return false
        }
        return remoteUser.isVideoMuted
    }
    ...
    func isAudioMuted(forUser userId: String) -> Bool {
        ...
        return remoteUser.isAudioMuted
    }
    
    func muteRemoteVideo(forUser userId: String, muted: Bool) {
        guard let remoteUser = roomUserMap[userId] as? LiveRemoteUser else { return }
