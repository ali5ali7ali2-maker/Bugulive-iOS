...
+ (void)showAnimationOfSusWindowSizeBlock:(void(^)(BOOL finished))block
{
    ...
    if(!SUS_WINDOW.isSmallSusWindow)
    {
        [SuspenionWindow closeSanwichLayerOfNetRootVCStr:@"BGTabBarController" complete:^(BOOL finished) {
    ...
    BGTLiveController  *tLiveC;
    BGKSYLiveController *ksyLiveC;
    UIView *oneLiveView;
    UIButton *closeLiveBtn;
    if(SUS_WINDOW.sdkType == FW_LIVESDK_TYPE_TXY)
    {
        tLiveC = (BGTLiveController *)SUS_WINDOW.rootViewController.childViewControllers[0];
    ...
    {
        //        ksyLiveC = (BGKSYLiveController *)SUS_WINDOW.rootViewController.childViewControllers[0];
        ksyLiveC = (BGKSYLiveController *)SUS_WINDOW.threeFWKSYLiveController;
...
+ (void)closeSanwichLayerOfNetRootVCStr:(NSString *)nextRootVCStr complete:(void(^)(BOOL finished))block
{
    ...
    {
        APP_DELEGATE.window.rootViewController = [BGTabBarController sharedInstance];
    }
...
+ (void)resetSusWindowPramaWhenLiveClosedComplete:(void(^)(BOOL finished))block
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if ([[[BGTabBarController sharedInstance] selectedViewController].childViewControllers[0] isKindOfClass:[HMHomeViewController class]])
        {
