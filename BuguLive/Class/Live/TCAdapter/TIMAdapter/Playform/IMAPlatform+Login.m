...
#define kEachKickErrorCode1 6202
- (void)login:(TIMLoginParam *)param succ:(TIMLoginSucc)succ fail:(TIMFail)fail
{
    ...
    [[V2TIMManager sharedInstance] login:param.identifier userSig:param.userSig succ:^{
        ...
        if (code == kEachKickErrorCode || code ==  kEachKickErrorCode1)
        {
            // 互踢重联
            // 重新再登录一次
...
{
    [BGHUDHelper alert:ASLocalizedString(@"您的账号在其他设备登录，如果不是您的操作，请及时修改密码") action:^{
