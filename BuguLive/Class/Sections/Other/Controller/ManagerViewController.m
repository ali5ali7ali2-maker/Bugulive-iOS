...
- (void)initFWUI
{
    ...
    self.navigationItem.title = ASLocalizedString(@"管理员列表");
    
    self.headLabel = [[UILabel alloc]initWithFrame:CGRectMake(kRealValue(15), 0, kScreenW, 40)];
    self.headLabel.backgroundColor = kBackGroundColor;
//    self.headLabel.textAlignment = NSTextAlignmentCe;
    ...
    UIView *line = [[UIView alloc]initWithFrame:CGRectMake(0, self.headLabel.bottom, kScreenW, kRealValue(10))];
    line.backgroundColor = [UIColor groupTableViewBackgroundColor];
    [self.view addSubview:line];
    
    self.tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, line.bottom, kScreenW, kScreenH - line.bottom)];
    ...
    [BGMJRefreshManager refresh:self.tableView target:self headerRereshAction:@selector(loadNetData) footerRereshAction:nil];
}
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
     {
         ...
         if ([responseJson toInt:@"status"] == 1)
         {
             ...
             self.headLabel.text = [NSString stringWithFormat:ASLocalizedString(@"当前管理员(%d/%d)"),self.cur_num,self.max_num];
        ...
        [self.httpsManager POSTWithParameters:dictM SuccessBlock:^(NSDictionary *responseJson)
         {
             ...
             if ([responseJson toInt:@"status"] == 1)
             {
                 if (self.has_admin == 0)
                 {
                     ...
                     [self.tableView reloadData];
                     [FanweMessage alert:ASLocalizedString(@"取消管理员成功")];
                 }
             }
             
         } FailureBlock:^(NSError *error)
         {
             [FanweMessage alert:ASLocalizedString(@"管理员设置失败，请重新操作")];
         }];
    }
}
