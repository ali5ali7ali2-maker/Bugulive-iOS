...
- (void)creatMainView
{
    self.headLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, kStatusBarHeight, kScreenW / 3 * 2, 44)];
    self.headLabel.backgroundColor = kWhiteColor;
    self.headLabel.textColor = kAppGrayColor2;
    if (self.type == 0)
    {
        self.headLabel.text   = ASLocalizedString(@"请选择要踢出的观众");
        
    }else
    {
        self.headLabel.text      = ASLocalizedString(@"请选择要邀请的好友");
    }
    self.headLabel.textAlignment = NSTextAlignmentCenter;
    self.headLabel.font      = [UIFont systemFontOfSize:14];
    self.headLabel.centerX = kScreenW / 2;
    [self.view addSubview:self.headLabel];
    
    self.navBackBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.navBackBtn setImage:[UIImage imageNamed:@"com_arrow_vc_back"] forState:UIControlStateNormal];
    self.navBackBtn.frame = CGRectMake(10, kStatusBarHeight, kRealValue(35), kRealValue(35));
    [self.navBackBtn addTarget:self action:@selector(clickBack:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [BGMJRefreshManager refresh:self.myTableView target:self headerRereshAction:@selector(headerReresh) footerRereshAction:@selector(footerReresh)];
    
    self.backView = [[UIView alloc]initWithFrame:CGRectMake(0, kScreenH-60- FD_Bottom_SafeArea_Height, kScreenW, 60)];
    self.backView.backgroundColor = kWhiteColor;
    [self.view addSubview:self.backView];
    self.backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.backButton.frame = CGRectMake(20,10, kScreenW-40, 40);
    [self.backButton setTitle:ASLocalizedString(@"取消")forState:UIControlStateNormal];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.idArray.count)
    {
        if (self.type == 0)//踢出好友
        {
            [self.backButton setTitle:ASLocalizedString(@"踢出观众")forState:UIControlStateNormal];
        }else
        {
            [self.backButton setTitle:ASLocalizedString(@"邀请好友")forState:UIControlStateNormal];
        }
    }else
    {
        [self.backButton setTitle:ASLocalizedString(@"取消")forState:UIControlStateNormal];
