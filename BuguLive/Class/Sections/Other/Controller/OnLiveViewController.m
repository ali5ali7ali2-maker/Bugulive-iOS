...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = ASLocalizedString(@"我的回播");
...
- (void)creatView{
    ...
    [BGMJRefreshManager refresh:_myTableView target:self headerRereshAction:@selector(headerReresh) footerRereshAction:@selector(footerReresh)];
}
...
- (void)loadNetDataWithPage:(int)page
{
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson) {
        ...
        if ([responseJson toInt:@"status"] == 1)
        {
            ...
            _liveLabel.text = [NSString stringWithFormat:ASLocalizedString(@"%d个精彩回放"),_count];
            ...
            if (array)
            {
                if (array.count > 0)
                {
                    for (NSDictionary *dict in array)
                    {
                        ...
                        if([dict objectForKey:@"live_image"]){
                            dic=[NSMutableDictionary dictionaryWithDictionary:dict];
                        }else{
                            dic=[NSMutableDictionary dictionaryWithDictionary:dict];
                            [dic setObject:@"" forKey:@"live_image"];
                        }
                                               //UserModel *model = [UserModel mj_objectWithKeyValues:dict];SenderModel
                        SenderModel *model = [SenderModel mj_objectWithKeyValues:dic];
                    }
                }
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_dataArray.count > 0 && _dataArray.count > indexPath.row)
    {
        if ([self checkUser:[IMAPlatform sharedInstance].host])
        {
            ...
            [[LiveCenterManager sharedInstance] showLiveOfAudienceLiveofTCShowLiveListItem:liveRoom modelArr:_dataArray isSusWindow:NO isSmallScreen:NO block:^(BOOL isFinished) {
        ...
        [self.httpsManager POSTWithParameters:dictM SuccessBlock:^(NSDictionary *responseJson)
         {
             ...
             if ([responseJson toInt:@"status"] == 1)
             {
                 [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"删除回播视频成功")];
             }
             
         } FailureBlock:^(NSError *error)
         {
             [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"删除回播视频失败，请重新操作")];
         }];
    }
}
