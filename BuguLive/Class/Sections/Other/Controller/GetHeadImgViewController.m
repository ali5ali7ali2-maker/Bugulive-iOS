...
-(UIImageView *)headImgView{
    if (!_headImgView) {
        ...
        _headImgView = [[UIImageView alloc]initWithFrame:CGRectMake(0, kStatusBarHeight, kScreenW, kScreenW)];
        _headImgView.image = kDefaultPreloadHeadImg;
...
- (void)initFWUI
{
    ...
    [backBtn setImage:[UIImage imageNamed:@"com_arrow_vc_back_2"] forState:UIControlStateNormal];
    ...
    self.photoButton.backgroundColor = RGB(118, 59, 243);
    self.phoneChoseButton.layer.cornerRadius = self.phoneChoseButton.frame.size.height/2;
    self.phoneChoseButton.backgroundColor = RGB(118, 59, 243);
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if (mediaType)
    {
        ...
        if (self.clickHeadImageBlock) {
            self.clickHeadImageBlock(image);
...
- (void)getHeadNet
{
    ...
    if (self.BuguLive.appModel.open_sts)
    {
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
     {
         ...
         {
             [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"头像更换成功")];
         }
