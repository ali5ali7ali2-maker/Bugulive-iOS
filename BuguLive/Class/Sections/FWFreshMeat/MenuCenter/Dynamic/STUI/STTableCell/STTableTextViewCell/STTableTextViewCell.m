...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    if ([textView.text isEqualToString: ASLocalizedString(@"这一刻你的想法")]) {
        textView.text = @"";
...
-(void)textViewDidEndEditing:(UITextView *)textView{
    if (textView.text.length == 0) {
        textView.text = ASLocalizedString(@"这一刻你的想法");
        textView.textColor = [UIColor colorWithHexString:@"#ABABAB"];
    }
...
-(BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text{
    ...
    if (caninputlen >=0){
        ...
        if (rg.length >0){
            ...
            self.showWordsNumLab.text = @"100/100";
        }
        return NO;
    }
    return YES;
...
-(void)textViewDidChange:(UITextView *)textView{
    ...
    //不让显示负数
    self.showWordsNumLab.text = [NSString stringWithFormat:@"%ld/100",  existTextNum];
