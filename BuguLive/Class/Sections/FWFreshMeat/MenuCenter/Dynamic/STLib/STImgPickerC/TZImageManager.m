    ...
    if (iOS8Later) {
        ...
        for (PHFetchResult *fetchResult in allAlbums) {
            for (PHAssetCollection *collection in fetchResult) {
                ...
                if ([collection.localizedTitle containsString:@"Deleted"] || [collection.localizedTitle isEqualToString:ASLocalizedString(@"最近删除")]) continue;
                if ([self isCameraRollAlbum:collection.localizedTitle]) {
        ...
        [self.assetLibrary enumerateGroupsWithTypes:ALAssetsGroupAll usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
            ...
            } else if ([name isEqualToString:@"My Photo Stream"] || [name isEqualToString:ASLocalizedString(@"我的照片流")]) {
                if (albumArr.count) {
...
- (void)savePhotoWithImage:(UIImage *)image completion:(void (^)(NSError *error))completion {
    ...
    if (iOS9Later) { // 这里有坑... iOS8系统下这个方法保存图片会失败 原来是因为PHAssetResourceType是iOS9之后的...
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            ...
            dispatch_sync(dispatch_get_main_queue(), ^{
                if (success && completion) {
                    ...
                    NSLog(ASLocalizedString(@"保存照片出错:%@"),error.localizedDescription);
                    if (completion) {
                        completion(error);
                    }
                }
        ...
        [self.assetLibrary writeImageToSavedPhotosAlbum:image.CGImage orientation:[self orientationFromImage:image] completionBlock:^(NSURL *assetURL, NSError *error) {
            if (error) {
                NSLog(ASLocalizedString(@"保存图片失败:%@"),error.localizedDescription);
                if (completion) {
                    completion(error);
                }
            } else {
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset completion:(void (^)(NSString *outputPath))completion {
    ...
    if ([presets containsObject:AVAssetExportPreset640x480]) {
        ...
        if ([supportedTypeArray containsObject:AVFileTypeMPEG4]) {
            ...
            NSLog(ASLocalizedString(@"No supported file types 视频类型暂不支持导出"));
            return;
        } else {
...
- (BOOL)isCameraRollAlbum:(NSString *)albumName {
    ...
    if (version >= 800 && version <= 802) {
        return [albumName isEqualToString:ASLocalizedString(@"最近添加")] || [albumName isEqualToString:@"Recently Added"];
    } else {
        return [albumName isEqualToString:@"Camera Roll"] || [albumName isEqualToString:ASLocalizedString(@"相机胶卷")] || [albumName isEqualToString:ASLocalizedString(@"所有照片")] || [albumName isEqualToString:@"All Photos"];
    }
}
