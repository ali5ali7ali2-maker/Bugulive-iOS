...
@implementation BGVideoMirrorCell
...
- (CGFloat)creatCellWithModel:(PersonCenterListModel *)model andRow:(int)row isVideo:(BOOL)isVideo
{
    ...
    if (isVideo)
    {
        nameString = [NSString stringWithFormat:ASLocalizedString(@"视频简介:%@"),model.content];
    ...
    }else
    {
        ...
        nameString = [NSString stringWithFormat:ASLocalizedString(@"写真简介:%@"),model.content];
    }
