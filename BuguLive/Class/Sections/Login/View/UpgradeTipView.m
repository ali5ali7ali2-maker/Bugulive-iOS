...
- (void)loginRewards
{
    ...
    _rewardView.exLabel.text = [NSString stringWithFormat:ASLocalizedString(@"经验+%@"),self.BuguLive.appModel.login_send_score];
    _upgradeView.upgradeLabel.text = [NSString stringWithFormat:ASLocalizedString(@"恭喜您升到%ld级！"),(long)self.BuguLive.appModel.new_level];
    ...
    if ([haveLanch boolValue] == YES )
    {
        ...
        {
            //是否有每日首次登入经验奖励
            if ([self.BuguLive.appModel.first_login isEqualToString:@"1"] )
            {
                [self promptRewardView];
            }
            //是否升级了
            if (self.BuguLive.appModel.new_level > 0 && self.BuguLive.appModel.open_upgrade_prompt == 1)
            {
...
{
    //既是每日首次登录，等级又提升了
    if ([self.BuguLive.appModel.first_login isEqualToString:@"1"] && self.BuguLive.appModel.new_level > 0 && self.BuguLive.appModel.open_upgrade_prompt == 1)
    {
        return YES;
    }
    else
