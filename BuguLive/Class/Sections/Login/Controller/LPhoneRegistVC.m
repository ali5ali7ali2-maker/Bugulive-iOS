...
- (void)initFWUI
{
    ...
    self.navigationItem.title = ASLocalizedString(@"手机注册");
    ...
    self.nameFiled.placeholder = ASLocalizedString(@"填写昵称");
...
- (IBAction)btnClick:(UIButton *)sender
{
    ...
    switch (sender.tag) {
        ...
        case 1: //选择男性
        {
            ...
            [_leftSexBtn setImage:[UIImage imageNamed:@"com_new_sex_male"] forState:UIControlStateNormal];
        }
            break;
        case 2://选择女性
        {
            ...
            [_rightSexBtn setImage:[UIImage imageNamed:@"com_new_sex_female"] forState:UIControlStateNormal];
        }
            break;
        case 3://下一步
        {
            ...
            {
                if (_urlString.length < 6 || [_urlString isEqualToString:@""] )
                {
                    [parmDict setObject:@"" forKey:@"head_image"];
                }
                else
            ...
            {
                [FanweMessage alert:ASLocalizedString(@"请输入你的昵称")];
                return;
            }
            ...
            [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
             {
                 ...
                 if ([responseJson toInt:@"status"] == 1)
                 {
                     ...
                     NSString *token = [NSString stringWithFormat:@"%@",responseJson[@"user_info"][@"token"]];
                     [GlobalVariables sharedInstance].token = token;
                     
                      //进入登录推荐用户页面
//                     LoginRecommendVC *phoneRegist = [[LoginRecommendVC alloc]init];
//
//
//                     [self presentViewController:phoneRegist animated:YES completion:nil];
                     [self hideMyHud];
                     if ([BGIMLoginManager sharedInstance].loginParam.identifier.length) {
