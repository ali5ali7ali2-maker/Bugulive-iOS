...
#import <QCloudCore/QCloudFCUUID.h>

@import FirebaseCore;
@import GoogleSignIn;
@import FirebaseAuth;

@interface BogoJHLogin ()<UITextFieldDelegate,UITextViewDelegate>
...
@property (weak, nonatomic) IBOutlet QMUIFillButton *googleBtn;
...
@property (weak, nonatomic) IBOutlet QMUIFillButton *wxBtn;
...
@property (weak, nonatomic) IBOutlet QMUIFillButton *facebookBtn;
...
//    [self.selectBtn setImage:[UIImage imageNamed:@"mg_circle_select_normal"] forState:UIControlStateNormal];
    ...
    //设置logo为appicon
//    NSString *iconName = [[[NSBundle mainBundle] infoDictionary] valueForKeyPath:@"CFBundleIcons.CFBundlePrimaryIcon.CFBundleIconFiles"][0];

    ViewRadius(self.logo, 4);
    self.logo.image = [UIImage imageNamed:@"海外直播logo120"];
...
- (IBAction)clickConfirmBtn:(UIButton *)sender {
    ...
    if (_loginType == BOGO_LOGIN_TYPE_CODE) {
        ...
        BogoJHLogin *vc = [BogoJHLogin new];
...
- (IBAction)clickGoogleLogin:(id)sender {
    ...
    [GIDSignIn.sharedInstance signInWithPresentingViewController:self
          ...
          [signInResult.user refreshTokensIfNeededWithCompletion:^(GIDGoogleUser * _Nullable user,
                                                                   NSError * _Nullable error) {
              if (error) {
                  [BGHUDHelper alert:[NSString stringWithFormat:@"error2 %@",error.description]];
                  [self hideMyHud];
                  return;
              }
              if (user == nil) {
                  [BGHUDHelper alert:[NSString stringWithFormat:@"user is nil"]];
                  [self hideMyHud];
                  return;
              }
              ...
              [parmDict setObject:SafeStr([QCloudFCUUID uuidForDevice]) forKey:@"device_id"];
              ...
              [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
               {
                   ...
                   if ([responseJson toInt:@"status"] == 1)
                   {
                       ...
                       [[BGIMLoginManager sharedInstance] getUserSig:^{
                           ...
                           [self hideMyHud];
                           [BGHUDHelper alert:[NSString stringWithFormat:@"IM %@",errMsg]];
                       }];
                   }
                  else
                  {
                      [BGHUDHelper alert:[NSString stringWithFormat:@"retry login %@",[responseJson toString:@"error"]]];
...
- (IBAction)clickWxBtn:(UIButton *)sender {
    ...
    // BogoLoginViewController *login = [BogoLoginViewController new];
...
-(void)phoneCodeLogin{
    ...
    if([GlobalVariables sharedInstance].openFirebaseSMS)
    {
...
{
    BGMainWebViewController *tmpController;
    
    if ([[URL absoluteString] isEqualToString:@"action1://"]) {
        // ✅ 用户服务协议 - User Agreement
        tmpController = [BGMainWebViewController 
