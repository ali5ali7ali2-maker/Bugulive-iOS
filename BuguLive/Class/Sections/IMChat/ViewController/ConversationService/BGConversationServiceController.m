//
//  BGConversationServiceController.m
//  BuguLive
//
...
@interface BGConversationServiceController () <GiftViewDelegate, ExchangeCoinViewDelegate, RechargeViewDelegate, ExchangeViewDelegate, OtherChangeViewDelegate>
{
...
@implementation BGConversationServiceController

+ (BGConversationServiceController *)makeChatVCWith:(SFriendObj *)chattag isHalf:(BOOL)mbhalf
{
    BGConversationServiceController *chatvc = [[BGConversationServiceController alloc] initWithNibName:@"BGBaseChatController" bundle:nil];
...
- (void)viewDidLoad
{
    ...
    self.clickBGBlock = ^(BOOL isRefresh) {
...
- (void)willSendThisVoice:(NSURL *)voicepath duration:(NSTimeInterval)duration
{
    if (duration < 1)
    {
        ...
        [SVProgressHUD showErrorWithStatus:ASLocalizedString(@"说话时间太短，取消发送！")];
        return;
    }
...
- (void)msgClicked:(ConversationModel *)msgobj
{
    ...
    if (msgobj.mMsgType == 4)
    {
        if (!msgobj.mIsSendOut)
        {
            ...
            {
                //如果不包含游戏币就能跳转
                if (![msgobj.mGiftDesc containsString:ASLocalizedString(@"游戏币")] && ![msgobj.mGiftDesc containsString:self.BuguLive.appModel.diamond_name])
                {
...
- (void)cfgmoremoregiftview
{
    ...
    CGFloat _giftViewHeight = KGiftViewHeight;
//    MorePanH + MG_BOTTOM_MARGIN;
...
{
    //[SVProgressHUD showWithStatus:ASLocalizedString(@"正在发送礼物...")];
...
#pragma 加载半imMsgVC
//ykk 加载banVC
+ (BGConversationServiceController *)createIMMsgVCWithHalfWith:(SFriendObj *)friend_Obj form:(UIViewController *)full_VC
                                                      isRelive:(BOOL)sender
{
    //1.0
    BGConversationServiceController *imMsgChat_VC = [[BGConversationServiceController alloc] initWithNibName:@"BGBaseChatController"
                                                                                                      bundle:nil];
    //1.2
...
- (void)createExchangeCoinView
{
    ...
    if (!_exchangeView)
    {
        ...
        _exchangeView.frame = CGRectMake((kScreenW - 300) / 2, kScreenH, 270, 246);
...
- (void)displayExchangeView
{
    ...
    [UIView animateWithDuration:0.3
                     animations:^{
                         ...
                         if (self.mbhalf)
                         {
                             _exchangeView.frame = CGRectMake((kScreenW - 270) / 2, (kScreenH - 350) / 2, 270, 246);
                         }
                         else
                         {
                             _exchangeView.frame = CGRectMake((kScreenW - 270) / 2, (kScreenH - 450) / 2, 270, 246);
                         }
                     }];
}
...
{
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:ASLocalizedString(@"赠送游戏币")message:nil
                                                                      preferredStyle:UIAlertControllerStyleAlert];
    //确定:UIAlertActionStyleDefault
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:ASLocalizedString(@"确定")style:UIAlertActionStyleDestructive
                                                     handler:^(UIAlertAction *_Nonnull action) {
                                                         ...
                                                         {
                                                             [SVProgressHUD showInfoWithStatus:ASLocalizedString(@"请输入金额")];
                                                         }
    ...
    [alertController addTextFieldWithConfigurationHandler:^(UITextField *_Nonnull textField) {
        ...
        textField.placeholder = ASLocalizedString(@"请输入金额");
    }];

    //取消:UIAlertActionStyleDestructive
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:ASLocalizedString(@"取消")style:UIAlertActionStyleCancel
                                                         handler:^(UIAlertAction *_Nonnull action) {
                                                             [self clickedfacemore:NO];
                                                         }];
                                     ...
                                     SuccessBlock:^(NSDictionary *responseJson) {
                                         ...
                                         if ([responseJson toInt:@"status"] == 1)
                                         {
                                             if (sendView == _exchangeView)
                                             {
                                                 ...
                                                 _exchangeView.diamondLabel.text = [NSString stringWithFormat:ASLocalizedString(@"游戏币余额:%@"), _exchangeView.ticket];
                                             }
                                             else if (sendView == _sendDiamondView)
                                             {
                                                 NSString *coinStr = [responseJson toString:@"user_diamonds"];
                                                 _sendDiamondView.ticket = coinStr;
                                                 _sendDiamondView.diamondLabel.text = [NSString stringWithFormat:ASLocalizedString(@"钻石余额:%@"), _sendDiamondView.ticket];
                                             }
                                         }
                                     }
...
{
    //    [SVProgressHUD showWithStatus:ASLocalizedString(@"正在赠送游戏币...")];
    [self.mChatFriend sendCoinMsgWithCoin:coin
                                    block:^(SResBase *resb, SIMMsgObj *thatmsg) {
                                        if (!resb.msuccess)
                                        {

                                            [[BGHUDHelper sharedInstance] tipMessage:resb.mmsg];
                                        }
...
- (RechargeView *) rechargeView
{
    if (_rechargeView == nil)
    {
        ...
        self.mrechargeView = _rechargeView;
...
//        _rechargePopView.clickHideBlock = ^(BOOL isClick) {
//            self.mGiftView.hidden = YES;
//        };
    }
