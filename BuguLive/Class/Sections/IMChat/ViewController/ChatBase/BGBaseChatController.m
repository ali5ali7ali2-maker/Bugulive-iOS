...
@interface BGBaseChatController () <UITextViewDelegate, UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, AVAudioRecorderDelegate, AVAudioPlayerDelegate, UIGestureRecognizerDelegate, ChatBottomBarDelegate, ChatEmojiViewDelegate, ChatMoreViewDelegate>
...
@implementation BGBaseChatController
{
...
//        mtableview
//        make.bottom.equalTo(self.view).offset(-MG_BOTTOM_MARGIN);
//
    ...
    [self.mtableview mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.chatBar).with.offset(-kChatBarMinHeight - MG_BOTTOM_MARGIN);
    }];
}
...
- (UIView *)mtopview
{
    if (!_mtopview)
    {
        ...
        _mtoptitle.textColor = [BGUtils colorWithHexString:@"333333"];
...
- (ChatBottomBarView *)chatBar
{
    if (!_chatBar)
    {
        NSMutableArray *allmore = [@[
            @[ASLocalizedString(@"照片"), @"ic_private_chat_more_photo"],
            @[ASLocalizedString(@"拍摄"), @"ic_private_chat_more_camera"],
            @[ASLocalizedString(@"礼物"), @"ic_private_chat_more_gift"],
        ] mutableCopy];
        if (self.BuguLive.appModel.open_send_coins_module == 1)
        {
            [allmore addObject:@[ASLocalizedString(@"赠送游戏币"), @"ic_private_chat_more_money"]];
        }
        if (self.BuguLive.appModel.open_send_diamonds_module == 1)
        {
            NSString *diamondsStr = [NSString stringWithFormat:ASLocalizedString(@"赠送%@"),self.BuguLive.appModel.diamond_name];
        ...
        self.chatBar.moreListArr = allmore;
        self.chatBar.moreView.delegate = self;
        self.chatBar.mbhalf = _mbhalf;
        if (self.BuguLive.appModel.has_private_chat == 1)
        {
            if (self.BuguLive.appModel.private_letter_lv)
            {
                if ([[IMAPlatform sharedInstance].host getUserRank] >= self.BuguLive.appModel.private_letter_lv)
                {
                    //允许
        ...
        [self getMsgBefor:fmsg
                    block:^(NSArray *all) {
                        ...
                        if (all.count)
                        {
                            ...
                            if (msgCount == 0)
                            {
                                [self.mtableview scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:all.count - 1 inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:NO];
                            ...
                            else
                            {
                                ...
                                [self.mtableview scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:NO];
...
{
    NSInteger s = [self.mtableview numberOfSections];  //有多少组
    if (s<1) return;  //无数据时不执行 要不会crash
    NSInteger r = [self.mtableview numberOfRowsInSection:s-1]; //最后一组有多少行
    if (r<1) return;
    NSIndexPath *ip = [NSIndexPath indexPathForRow:r-1 inSection:s-1];  //取最后一行数据
    [self.mtableview scrollToRowAtIndexPath:ip atScrollPosition:UITableViewScrollPositionBottom animated:animated]; //滚动到最后一行
}
...
- (void)willReSendThisMsg:(ConversationModel *)msg
{
    ...
    //for testcode
    if ([NSStringFromClass([self class]) isEqualToString:NSStringFromClass([BGBaseChatController class])])
    {
    }
}
...
#pragma mark 声音cell最宽,最窄
    ...
    [retcell.mheadimg sd_setImageWithURL:[NSURL URLWithString:msgobj.mHeadImgUrl] placeholderImage:kDefaultPreloadHeadImg];
...
- (BOOL)realPlayVoice:(ConversationModel *)msg
{
    ...
    {
        [SVProgressHUD showErrorWithStatus:ASLocalizedString(@"播放该消息失败")];
        [self stopPlayVoice:msg];
        _nowplayingmsg = nil;
        return NO;
    }
...
//    self.chatBar.moreView.hidden = YES;
    ...
    if (self.mGiftView.superview == nil) [self.view addSubview:self.mGiftView];
//        [self.chatBar.moreView addSubview:self.mGiftView];
    ...
    [UIView animateWithDuration:0.1f
        ...
        fff.origin.y = kScreenH - KGiftViewHeight - MG_BOTTOM_MARGIN * 2;
//        0;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info
{
    ...
    if (newImg == nil)
    {
        [SVProgressHUD showErrorWithStatus:ASLocalizedString(@"图片无效!请重新选择")];
        return;
    }
...
{
    //TODO ASLocalizedString(@"手指上滑，取消发送")//self.recordPhase = AudioRecordPhaseRecording;
    [self showRecSV:3];
}

- (void)onTouchRecordBtnDragOutside:(id)sender
{
    //TODO ASLocalizedString(@"松开手指，取消发送")//self.recordPhase = AudioRecordPhaseCancelling;
    [self showRecSV:2];
}
...
- (void)startRecordeAudio
{
    ...
    if ([session respondsToSelector:@selector(recordPermission)] &&
        session.recordPermission == AVAudioSessionRecordPermissionUndetermined)
    {
        [session requestRecordPermission:^(__unused BOOL granted) {
        }];
        return;
    }
    ...
    if (![self canRecord])
    {
        [SVProgressHUD showErrorWithStatus:ASLocalizedString(@"获取麦克风权限失败")];
        return;
    }
        ...
        ![session.category isEqualToString:AVAudioSessionCategoryRecord])
    {
        ...
        {
            [SVProgressHUD showErrorWithStatus:ASLocalizedString(@"录音失败")];
            return;
        }
...
- (void)chatBarFrameDidChange:(ChatBottomBarView *)chatBar shouldScrollToBottom:(CGFloat)keyBoardHeight showType:(FWChatBarShowType)showType showAnimationTime:(CGFloat)showAnimationTime
{
    ...
    {
        if (showType == FWChatBarShowTypeMore) {
            UIEdgeInsets contentInsets = UIEdgeInsetsMake(0.0, 0.0, 234, 0);
...
//            [_chatBar mas_updateConstraints:^(MASConstraintMaker *make) {
//                make.bottom.mas_equalTo(-self.chatBar.moreView.height);
//            }];
        }
        if (showType == FWChatBarShowTypeFace )
        {
            UIEdgeInsets contentInsets = UIEdgeInsetsZero;
//            UIEdgeInsetsMake(0.0, 0.0, 234, 0.0);
            _mtableview.scrollEnabled = YES;
            _mtableview.contentInset = contentInsets;
            _mtableview.scrollIndicatorInsets = contentInsets;
            [self scrollToBottomAnimated:YES animatedTime:showAnimationTime];
        ...
        else if (showType == FWChatBarShowTypeNothing)
        {
            ...
            _mtableview.contentInset = contentInsets;
            _mtableview.scrollIndicatorInsets = contentInsets;
//            [_chatBar.inputBarBackgroundView mas_updateConstraints:^(MASConstraintMaker *make) {
//                make.bottom.mas_equalTo(-MG_BOTTOM_MARGIN - 10);
//            }];
        }
        else if (showType == FWChatBarShowTypeKeyboard)
        {
//            UIEdgeInsets contentInsets = UIEdgeInsetsZero;
            UIEdgeInsets contentInsets = UIEdgeInsetsMake(0.0, 0.0, 20, 0);
            _mtableview.contentInset = contentInsets;
            _mtableview.scrollIndicatorInsets = contentInsets;
            [self scrollToBottomAnimated:YES animatedTime:showAnimationTime];
            [_chatBar.inputBarBackgroundView mas_updateConstraints:^(MASConstraintMaker *make) {
...
-(void)clickHide{
    self.chatBar.showType = FWChatBarShowTypeNothing;
...
- (void)chatMoreViewButton:(NSInteger)btnIndex
{
    ...
    else if (index == 1003)
    {
        ...
        else if (self.BuguLive.appModel.open_send_diamonds_module == 1)
        {
            [self sendDiamonds];
        }
    }
...
//    if ([touch.view isKindOfClass:[UIButton class]] || [touch.view isKindOfClass:[RechargeWayView class]] || [touch.view isKindOfClass:[RechargeView class]]) {//充值界面出来时，不隐藏giftview
//        return NO;
//    }
    
    if ([touch.view isKindOfClass:[UIButton class]] || [touch.view isKindOfClass:[ChatBarTextView class]]) {
        return NO;
    }
    
    if (self.mrechargeView.hidden == NO && self.mrechargeView) {
        self.mGiftView.hidden = NO;
        return NO;
    }
    
    if (!self.mGiftView.hidden && [touch.view isKindOfClass:NSClassFromString(@"UITableViewCellContentView")]) {
        [self hidenGiftView];
        return YES;
    }
    
    if ([touch.view isKindOfClass:[ChatEmojiView class]]) {
        return NO;
    }
