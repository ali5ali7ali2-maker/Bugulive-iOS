...
+ (NSString *)stringWithTimelineDate:(NSDate *)date {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [formatterYesterday setDateFormat:ASLocalizedString(@"昨天")];
    ...
    if (delta < -60 * 10) { // 本地时间有问题
        ...
        return ASLocalizedString(@"刚刚");
    } else if (delta < 60 * 60) { // 1小时内
        return [NSString stringWithFormat:ASLocalizedString(@"%d分钟前"), (int)(delta / 60.0)];
    } else if (date.isToday) {
        return [NSString stringWithFormat:ASLocalizedString(@"%d小时前"), (int)(delta / 60.0 / 60.0)];
    } else if (date.isYesterday) {
        return ASLocalizedString(@"昨天");
    } else if (date.month == now.month) {
        //几天前
        return [NSString stringWithFormat:ASLocalizedString(@"%d天前"), (int)(delta / 60 / 60 / 24)];
...
+ (NSString *)shortedNumberDesc:(NSUInteger)number {
    ...
    if (number <= 9999999) return [NSString stringWithFormat:NSLocalizedString(@"%d万",nil), (int)(number / 10000)];
    return [NSString stringWithFormat:NSLocalizedString(@"%d千万",nil), (int)(number / 10000000)];
}
