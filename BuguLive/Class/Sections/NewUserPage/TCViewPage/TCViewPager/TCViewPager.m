...
#define DefultLeftAndRightSpace  0
#define DefultTitlePageSpace     0
#define EditBtnW   30
...
- (void)calculate {
    ...
    if(!self.titleArrayLength.count) {
        ...
        for(NSString *title in _titleArray) {
            CGFloat width = kScreenW / self.titleArray.count;
...
//底部选中下划线
@property (nonatomic, strong) UIImageView *selectedBottomLine;
...
- (void)setSelectIndex:(NSInteger)index isClickBtn:(BOOL)isClickBtn
{
    ...
    if(floor(self.scrollView.contentOffset.x) == floor(index * self.width)) {
        self.selectedBottomLine.width = 20;
...
- (void)layoutSubviews{
    ...
    if(self.param.showSelectedBottomLine) {
        //创建菜单按钮下划线
        self.selectedBottomLine = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.pageHeaderControl.height -4, 20, 4)];
        self.selectedBottomLine.image = [UIImage imageNamed:@"选中"];
