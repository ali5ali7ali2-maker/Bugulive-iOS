...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (_whetherGame)
    {
        ...
        [[IMAPlatform sharedInstance].host getMyInfo:^(AppBlockModel *blockModel) {
            ...
            self.acountLabel.text =[NSString stringWithFormat:ASLocalizedString(@"账户:%@%@"),self.tickte,self.BuguLive.appModel.diamond_name];
...
- (void)viewDidLoad
{
    ...
    {
        ratio = @"1";       //兑换比例
        _diamondsLabel.text = ASLocalizedString(@"兑换游戏币");
        _rightTextField.text =ASLocalizedString(@"0游戏币");
        _leftTextfield.placeholder =[NSString stringWithFormat:ASLocalizedString(@"请输入%@金额"),self.BuguLive.appModel.diamond_name];
    }
    else
    {
        _diamondsLabel.text = [NSString stringWithFormat:ASLocalizedString(@"兑换%@"),self.BuguLive.appModel.diamond_name];
        _rightTextField.text = [NSString stringWithFormat:@"0%@",self.BuguLive.appModel.diamond_name];
        _acountLabel.text =[NSString stringWithFormat:ASLocalizedString(@"账户:%@%@"),_tickte,self.BuguLive.appModel.ticket_name];
        _leftTextfield.placeholder =[NSString stringWithFormat:ASLocalizedString(@"请输入兑换的%@金额"),self.BuguLive.appModel.ticket_name];
        [self requetRatio];
    }
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (_whetherGame) {
        _rightTextField.text= [NSString stringWithFormat:ASLocalizedString(@"%d游戏币"),rat];
    }
    else
    {
        _rightTextField.text= [NSString stringWithFormat:@"%d%@",rat,self.BuguLive.appModel.diamond_name];
    }
...
- (void)converButtonActon
{
    ...
    if(ratio.floatValue !=0){
        if (_rightTextField.text.floatValue == 0&&_leftTextfield.text.floatValue<1/ratio.floatValue) {
            [FanweMessage alert:ASLocalizedString(@"请输入足够的兑换金额")];
            return;
        }
    }
    //当输入金额 大于账户
    ...
    {
        [FanweMessage alert:ASLocalizedString(@"兑换金额不得大于账户余额")];
        return;
    }
    if (rat >0)
    {
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
     {
         ...
         if ([responseJson toInt:@"status"]==1)
         {
             ...
             {
                 [FanweMessage alert:ASLocalizedString(@"兑换失败")];
             }
         }
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
     {
         ...
         if ([responseJson toInt:@"status"]==1)
         {
             if ([_rightTextField.text intValue]>0)
             {
                 ...
                 [SVProgressHUD showSuccessWithStatus:ASLocalizedString(@"兑换成功")];
             }
             else
             {
                 [SVProgressHUD showErrorWithStatus:ASLocalizedString(@"兑换失败")];
             }
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
     {
         ...
         if ([responseJson toInt:@"status"] ==1)
         {
             ...
             if (ratio != nil&& ![ratio isEqual:[NSNull null]]) {
                 self.exchangeRate_Lab.text = [NSString stringWithFormat:ASLocalizedString(@"兑换比例: %@"),ratio];
             }
         }
