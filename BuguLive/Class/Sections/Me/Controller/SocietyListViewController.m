...
- (void)searchbarDisplay
{
    ...
    self.navigationItem.title = ASLocalizedString(@"公会列表");
    //画线
...
- (void)creatTabelView
{
    ...
    [BGMJRefreshManager refresh:_displayTabel target:self headerRereshAction:@selector(headerReresh) shouldHeaderBeginRefresh:NO footerRereshAction:@selector(footerReresh)];
}
...
- (void)applyWithSocietyListCell:(SocietyListCell *)cell
{
    ...
    FWWeakify(self)
    [FanweMessage alert:ASLocalizedString(@"提示")message:ASLocalizedString(@"是否申请加入该公会")destructiveAction:^{
