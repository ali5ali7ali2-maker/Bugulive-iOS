...
- (void)initFWUI
{
    ...
    [BGMJRefreshManager refresh:self.detailCollectionView target:self headerRereshAction:@selector(headerNew) footerRereshAction:@selector(footerNew)];
}
...
- (void)tableViewHeadMsgSetup
{
    ...
    else
    {
        ...
        {
            [self operationWithWidth:100 title:ASLocalizedString(@"入会申请中")imageStr:@""];
        }
        else if (self.type == 5)
        {
            [self operationWithWidth:100 title:ASLocalizedString(@"退会申请中")imageStr:@""];
        }
    }
    //从个人中心界面跳转过来执行以下逻辑
    if ([self.flagStr isEqualToString:mySociety] && self.type == 1)
    {
        ...
        { //公会未审核
            [self propertyWithBtnEnable:NO btnTitle:ASLocalizedString(@"状态：审核中")];
        }
        ...
        { //公会审核被拒绝
            [self propertyWithBtnEnable:YES btnTitle:ASLocalizedString(@"重新申请")];
        }
    }
}
/**
        ...
        [self.httpsManager POSTWithParameters:paramDict SuccessBlock:^(NSDictionary *responseJson)
         {
             ...
             if ([responseJson toInt:@"status"] == 1)
             {
                 ...
                 if (self.type == 1)
                 {
                     ...
                     {
                         NSString *inviteCodeStr = [NSString stringWithFormat:ASLocalizedString(@"邀请码：%@"),responseJson[@"society_code"]];
                         if (inviteCodeStr)
                         {
         ...
         {
             [BGMJRefreshManager endRefresh:self.detailCollectionView];
         }];
        [BGMJRefreshManager endRefresh:self.detailCollectionView];
}
...
- (void)configurationResponseJson:(NSDictionary *)responseJson
{
    ...
    head.presidentLbl.text       = [NSString stringWithFormat:ASLocalizedString(@"会长：%@"),[responseJson toString:@"society_chairman"]];
...
- (void)societyTotalNumberOfPeople:(int)number
{
    ...
    head.societyTotalLbl.text         = [NSString stringWithFormat:ASLocalizedString(@"公会人数：%d"),number];
...
{
    NSArray *arrayTitle;
    NSString *anchorStr = [NSString stringWithFormat:ASLocalizedString(@"主播")];
    NSString *fansStr   = [NSString stringWithFormat:ASLocalizedString(@"粉丝")];
    NSString *joinStr   = [NSString stringWithFormat:ASLocalizedString(@"成员申请")];
    NSString *outStr    = [NSString stringWithFormat:ASLocalizedString(@"退出申请")];
    ...
    if (segmentControl.selectedSegmentIndex == 0)
    {
        ...
        if (self.type == 1)//查看的人身份是会长
        {
            ...
            if (model.user_position.intValue == 1 && model.live_in.intValue == 0) {
                items = @[MMItemMake(ASLocalizedString(@"查看详情"), MMItemTypeNormal, ^(NSInteger index)
                                     {
            ...
            else if (model.user_position.intValue == 0)
            {
                ...
                {
                    items = @[MMItemMake(ASLocalizedString(@"观看直播"), MMItemTypeNormal, ^(NSInteger index)
                                         {
                                             [self viewLivingWithIndex:indexPath.row];
                                         }),MMItemMake(ASLocalizedString(@"查看详情"), MMItemTypeNormal, ^(NSInteger index)
                                                       {
                                                           [self jumpOffToHomePageWithModel:model];
                                                       }),MMItemMake(ASLocalizedString(@"踢出公会"), MMItemTypeNormal, ^(NSInteger index)
                                                                     {
...
- (void)societyBgClick
{
    if (!(self.type == 1))
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"你没有操作权限")];
        return;
    }
    else if (self.type == 1 && self.mygh_status == 0 && [self.flagStr isEqualToString:mySociety])
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"公会正在审核中")];
        return;
    }
...
- (void)tiantuanOperationClick
{
    ...
    {
        FWWeakify(self)
        [FanweMessage alert:ASLocalizedString(@"退出公会")message:[NSString stringWithFormat:ASLocalizedString(@"是否退出公会：%@？"),head.societyNameLbl.text] destructiveAction:^{
            ...
            [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson) {
                ...
                if ([responseJson toInt:@"status"]==1)
                {
                    [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"退会申请已提交")];
                    [self operationWithWidth:100 title:ASLocalizedString(@"退会申请中")imageStr:@""];
                    self.type = 5;
                }
            } FailureBlock:^(NSError *error) {
    ...
    else if (self.type == 3)
    {
        ...
        FWWeakify(self)
        [FanweMessage alert:ASLocalizedString(@"加入公会")message:[NSString stringWithFormat:ASLocalizedString(@"是否加入公会：%@？"),head.societyNameLbl.text] destructiveAction:^{
            ...
            [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson) {
                ...
                if ([responseJson toInt:@"status"]==1)
                {
                    [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"入会申请已提交")];
                    [self operationWithWidth:100 title:ASLocalizedString(@"入会申请中")imageStr:@""];
                    self.type = 4;
                }
            } FailureBlock:^(NSError *error) {
    ...
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"入会申请中")];
        return;
    }
    else if (self.type == 5)
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"退会申请中")];
        return;
    }
    ...
    if (head.OperationBtn.currentTitle.length > 3)
    {
        ...
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"已复制")];
    }
}
...
{
    FWWeakify(self)
    [FanweMessage alert:ASLocalizedString(@"踢出成员")message:ASLocalizedString(@"是否踢出该成员？")destructiveAction:^{
...
- (void)viewLivingWithIndex:(NSInteger)index
{
    ...
    if ([self checkUser:[IMAPlatform sharedInstance].host])
    {
        ...
        [[LiveCenterManager sharedInstance]  showLiveOfAudienceLiveofTCShowLiveListItem:item modelArr:self.sociatyDetailArray isSusWindow:isSusWindow isSmallScreen:NO block:^(BOOL isFinished) {
...
- (void)refuseOfSociety:(ApplicantCell *)cell
{
    ...
    {
        FWWeakify(self)
        [FanweMessage alert:ASLocalizedString(@"拒绝加入公会")message:ASLocalizedString(@"是否拒绝该成员加入公会？")destructiveAction:^{
    ...
    {
        FWWeakify(self)
        [FanweMessage alert:ASLocalizedString(@"拒绝退出公会")message:ASLocalizedString(@"是否拒绝该成员退出公会？")destructiveAction:^{
...
- (void)agreeOfSociety:(ApplicantCell *)cell
{
    ...
    {
        FWWeakify(self)
        [FanweMessage alert:ASLocalizedString(@"同意加入公会")message:ASLocalizedString(@"是否同意该成员加入公会？")destructiveAction:^{
    ...
    {
        FWWeakify(self)
        [FanweMessage alert:ASLocalizedString(@"同意退出公会")message:ASLocalizedString(@"是否同意该成员退出公会？")destructiveAction:^{
