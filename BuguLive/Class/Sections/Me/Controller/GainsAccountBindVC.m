...
- (void)viewDidLoad
{
    ...
    self.title = ASLocalizedString(@"账号绑定");
...
//    self.btnAffirm.layer.borderColor = kAppMainColor.CGColor;
    [self.btnAffirm setBackgroundColor:RGB(118, 59, 243)];
...
- (IBAction)onClick:(id)sender {
    ...
    if ([_tfPayName.text length] == 0) {
        ...
        [FanweMessage alert:ASLocalizedString(@"请输入支付宝名称")];
        return;
    }
    
    if ([_tfPayAccount.text length] == 0) {
        
        [FanweMessage alert:ASLocalizedString(@"请输入支付宝账号")];
        return;
    }
