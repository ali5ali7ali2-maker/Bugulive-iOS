...
@property (nonatomic, strong) UIButton * familyMemberBtn;//公会成员
...
- (void)selectButton
{
    ...
    //    NSString * familyMemberStr = [NSString stringWithFormat:ASLocalizedString(@"公会成员(%d)"),125];
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson) {
        ...
        if (_state == 1)
        {
            ...
            if (_isFamilyHeder == 1&& [_searchType isEqualToString:@"1"]) {
                NSString * familyMemberStr = [NSString stringWithFormat:ASLocalizedString(@"公会成员(%@)"),[responseJson objectForKey:@"rs_count"]];
...
{
    FWWeakify(self)
    [FanweMessage alert:nil message:ASLocalizedString(@"是否踢出该成员")destructiveAction:^{
        ...
        [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson) {
            ...
            if ([responseJson toInt:@"status"]==1)
            {
                ...
                self.familyNumber--;
                NSString * familyMemberStr = [NSString stringWithFormat:ASLocalizedString(@"公会成员(%d)"),self.familyNumber];
...
{
    FWWeakify(self)
    [FanweMessage alert:nil message:ASLocalizedString(@"是否同意该成员加入公会")destructiveAction:^{
...
{
    FWWeakify(self)
    [FanweMessage alert:nil message:ASLocalizedString(@"是否拒绝该成员加入公会")destructiveAction:^{
