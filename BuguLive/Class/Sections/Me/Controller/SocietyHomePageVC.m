...
- (void)initFWUI
{
    ...
    [BGMJRefreshManager refresh:self.societyHPCollectionView target:self headerRereshAction:@selector(refreshHeader) footerRereshAction:@selector(refreshFooter)];
}
...
- (void)loadDataWithPage:(int)page
{
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson) {
        ...
        if ([responseJson toInt:@"status"] == 1)
        {
            ...
            [BGMJRefreshManager endRefresh:self.societyHPCollectionView];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(nonnull NSIndexPath *)indexPath
{
    ...
    cell.presidentLbl.text = [NSString stringWithFormat:ASLocalizedString(@"会长：%@"),listModel.society_chairman];
    cell.numberLbl.text = [NSString stringWithFormat:ASLocalizedString(@"人数：%@"),listModel.society_user_count];
    return cell;
}
