...
- (void)initFWUI
{
    ...
    {
        self.ossManager = [[BGOssManager alloc]initWithDelegate:self];
    }
    self.dataArray = [[NSMutableArray alloc]init];
    self.typeArray = [[NSMutableArray alloc]init];
    self.recommendType = 0;
    self.recommendHeight = 0;
    self.navigationItem.title = ASLocalizedString(@"认证");
    ...
    self.promptLabel.text = ASLocalizedString(@"带星号项为必填项,为了保证您的利益,请如实填写");
    self.promptLabel.font = [UIFont systemFontOfSize:13];
    self.promptLabel.textColor = kAppGrayColor1;
    [self.view addSubview:self.promptLabel];
    
    self.basicInfoView  = [self creatViewWithStr:ASLocalizedString(@"基本资料")];
    self.idNewsView     = [self creatViewWithStr:ASLocalizedString(@"认证消息")];
    self.idAuthenView   = [self creatViewWithStr:ASLocalizedString(@"身份认证")];
    self.recommendView  = [self creatViewWithStr:ASLocalizedString(@"推荐人信息")];
    ...
    label1.text = ASLocalizedString(@"公会邀请码");
    ...
    label2.text = ASLocalizedString(@"(选填)");
    ...
    self.inviteCodeFiled.placeholder = ASLocalizedString(@"请输入您的公会邀请码");
    ...
    [_sumitBtn setTitle:ASLocalizedString(@"提交")forState:UIControlStateNormal];
    ...
    [BGMJRefreshManager refresh:_myTableView target:self headerRereshAction:@selector(refresherOfFocusOn) footerRereshAction:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == AuthenticationZeroSection)
    {
        ...
        cell.myNameLabel.text = [NSString stringWithFormat:ASLocalizedString(@"昵称：%@"),self.nameString];
        cell.mySexLabel.text = [NSString stringWithFormat:ASLocalizedString(@"性别：%@"),[self.sexString intValue] == 1 ? ASLocalizedString(@"男"): ASLocalizedString(@"女")];
        return cell;
    }else if (indexPath.section == AuthenticationOneSection)
    {
    ...
    }else if (indexPath.section == AuthenticationFourSection)
    {
        ...
        if (!cell)
        {
            ...
            [cell.contentView addSubview:self.inviteCodeView];
        }
    ...
    else
    {
        ...
        if (!cell)
        {
            ...
            cell.accessoryType = UITableViewCellAccessoryNone;
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            [cell.contentView addSubview:self.sumitBtn];
        }
        return cell;
    }
}
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    if (textField == _threeCell.contactTextFiled)
    {
        ...
        {
            [FanweMessage alertHUD:ASLocalizedString(@"请输入电话号码")];
            return;
        }
        if (_threeCell.contactTextFiled.text.length < 1)
        {
            [FanweMessage alertHUD:ASLocalizedString(@"请输入11位电话号码")];
            return;
        }
//        if ([_threeCell.contactTextFiled.text isTelephone])
//        {
//            [FanweMessage alertHUD:ASLocalizedString(@"请输入正确的电话号码")];
//            return;
//        }
    }
}
...
- (void)getIdentificationWithCell:(ThirdTableViewCell *)myCell
{
    if (!_typeSheet)
    {
        UIActionSheet  *typeSheet = [[UIActionSheet alloc] initWithTitle:ASLocalizedString(@"认证类型")delegate:self cancelButtonTitle:nil destructiveButtonTitle:nil otherButtonTitles:nil];
        for (int i=0; i < [_typeArray count]; i ++)
        {
            cuserModel *model = [_typeArray objectAtIndex:i];
            [typeSheet addButtonWithTitle:model.name];
        }
        [typeSheet addButtonWithTitle:ASLocalizedString(@"取消")];
...
- (void)submitAction
{
    ...
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请选择认证类型")];
        return;
    }
    if (_threeCell.nameTextFiled.text.length < 1)
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请输入真实姓名")];
        return;
    }
    if (_threeCell.contactTextFiled.text.length < 1)
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请输入联系方式")];
        return;
    }
//    if (_is_show_identify_number == 1)
//    {
...
//        {
//            [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请输入18位有效的身份证号码")];
//            return;
//        }
//    }
    ...
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请上传身份证正面照")];
        return;
    }
    if (_urlString2.length < 1)
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请上传身份证反面照")];
        return;
    }
    if (_urlString3.length < 1)
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请上传手持身份证正面照")];
        return;
    }
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    else if (actionSheet == self.recommendSheet)
    {
        if (buttonIndex < self.recommendSheet.numberOfButtons - 1)
        {
            ...
            if (model.typeID>1)
            {
                _recommendCell.recommendField.placeholder = [NSString stringWithFormat:ASLocalizedString(@"请输入%@"),model.name];
            }
            [_myTableView reloadData];
        }
    }
...
- (void)uploadImageWithUrlStr:(NSString *)imageUrlStr withUploadStateCount:(int)stateCount
{
    ...
    if (stateCount == 0)
    {
        ...
        }else if (_tagCount == 1)
        {
            ...
            [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"身份证反面照片上传成功")];
        }else if (_tagCount == 2)
        {
            _urlString3 = [NSString stringWithFormat:@"./%@",_timeString];
            [_fourCell.headImgView3 sd_setImageWithURL:[NSURL URLWithString:imgString]];
            [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"手持身份证正面照上传成功")];
        }
    }else
    {
...
- (void)choseRecommendTypeWithRecommendViewCell:(RecommendTableViewCell *)recommendViewCell
{
    if (!_recommendSheet)
    {
        UIActionSheet * recommendSheet = [[UIActionSheet alloc] initWithTitle:ASLocalizedString(@"推荐人信息类型")delegate:self cancelButtonTitle:nil destructiveButtonTitle:nil otherButtonTitles:nil];
        ...
        [recommendSheet addButtonWithTitle:ASLocalizedString(@"取消")];
