...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title =ASLocalizedString(@"领取记录");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([model.is_pay isEqualToString:@"1"]) {
        cell.moneyLable.text =[NSString stringWithFormat:ASLocalizedString(@"%@"),model.money];
        cell.timeLabel.text =model.create_time;
        cell.recordStateLable.text =ASLocalizedString(@"兑换中");
        
    }
    else{
        cell.moneyLable.text =[NSString stringWithFormat:ASLocalizedString(@"%@元"),model.money];
        cell.timeLabel.text =model.pay_time;
        cell.recordStateLable.text =ASLocalizedString(@"兑换成功");
    }
    return cell;
}
//添加标头中的内容
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (headerView == nil) {
        ...
        label.text = [NSString stringWithFormat:ASLocalizedString(@"累计领取:%@元"),totalMoney];
    ...
    [_httpManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
     {
         if ([responseJson toInt:@"status"]==1) {
            ...
            totalMoney = [responseJson objectForKey:@"total_money"];
             ...
             NSArray *listArr = [NSArray modelArrayWithClass:[RecordModel class] json:[responseJson objectForKey:@"list"]];
...
//                 }
//             }
             [recordArray addObjectsFromArray:listArr];
              [self creatTabel];
         }
