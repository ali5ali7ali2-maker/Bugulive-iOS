...
-(void)createView
{
    ...
    self.navigationItem.title = ASLocalizedString(@"公会资料修改");
    ...
    //    _nameTextView.text =ASLocalizedString(@"请输入公会名称");
    ...
    //    _desTextView.text = ASLocalizedString(@"请输入公会宣言");
...
-(void)createBtn
{
    ...
    [_ensureBtn setTitle:ASLocalizedString(@"确认")forState:UIControlStateNormal];
...
{
    if ([_nameTextView.text isEqualToString:ASLocalizedString(@"请输入公会名称")]&&[_nameTextView.textColor isEqual:[UIColor lightGrayColor]]) {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请填写公会名称")];
        return;
    }
    if ([_desTextView.text isEqualToString:ASLocalizedString(@"请输入公会宣言")]&&[_desTextView.textColor isEqual:[UIColor lightGrayColor]]) {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请填写公会宣言")];
        return;
    }
    ...
    if (_type==0) {
        if (_urlStr.length<1) {
            [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请编辑头像")];
            return;
        }
        ...
        [[BGHUDHelper sharedInstance] syncLoading:ASLocalizedString(@"正在创建公会请等待")];
    ...
    else if (_type==1) // 如果是修改公会资料
    {
        ...
        [[BGHUDHelper sharedInstance] syncLoading:ASLocalizedString(@"正在上传修改请等待")];
        [self.httpsManager POSTWithParameters:mDict SuccessBlock:^(NSDictionary *responseJson) {
...
-(void)textViewDidEndEditing:(UITextView *)textView
{
    ...
    else if (textView==_desTextView)
    {
        if (_desTextView.text.length<1) {
            _desTextView.text = ASLocalizedString(@"请输入公会宣言");
            _desTextView.textColor = [UIColor lightGrayColor];
            [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请输入公会宣言")];
            _ensureBtn.enabled = NO;
            _ensureBtn.backgroundColor = kAppGrayColor3;
        }
        else
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    if (_desTextView.text.length > 140) {
        ...
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"超过最大字数限制")];
    }
    return YES;
}
