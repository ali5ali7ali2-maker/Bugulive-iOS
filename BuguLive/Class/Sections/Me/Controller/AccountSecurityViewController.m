...
{
    [super viewDidLoad];
    self.title =ASLocalizedString(@"帐号与安全");
...
- (void)getNetAccAndSecWorking
{
    ...
    [_httpManager POSTWithParameters:dict SuccessBlock:^(NSDictionary *responseJson) {
        ...
        {
            self.asView.SecutyLable.text = ASLocalizedString(@"安全等级:高");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([self.model.is_security isEqualToString:@"0"])
    {
        PhoneLoginViewController *phoneLogin = [[PhoneLoginViewController alloc]initWithNibName:NSStringFromClass([PhoneLoginViewController class]) bundle:[NSBundle mainBundle]];
        phoneLogin.LSecBPhone = YES;
        [self.navigationController pushViewController:phoneLogin animated:YES];
    }
    else
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"您已绑定手机")];
