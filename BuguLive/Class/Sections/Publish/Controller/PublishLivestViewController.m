...
@interface PublishLivestViewController ()<publishViewDelegate, UINavigationControllerDelegate,UIImagePickerControllerDelegate,OssUploadImageDelegate, UIActionSheetDelegate,UITextViewDelegate,UITextFieldDelegate>
...
//    self.liveView.textView.delegate = self;
    ...
    [self.liveView.voiceBtn addTarget:self action:@selector(voiceClick) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)selectedTheClassifyDelegate
{
    ...
    alterVC.view.tintColor = kAppMainColor;
    ...
    [alterVC addAction:action];
    if (alterVC.popoverPresentationController) {
        alterVC.popoverPresentationController.sourceView = self.liveView.topView.classifyBtn;
        alterVC.popoverPresentationController.sourceRect = self.liveView.topView.classifyBtn.bounds;
    }
...
- (void)avAuthorizationCamera
{
    ...
    switch (status)
    {
        ...
        case AVAuthorizationStatusRestricted:
            ...
            [self showAlertViewCAndMessageStr:ASLocalizedString(@"Please enable camera access in Settings > Privacy > Camera")];
            break;
        default:
            break;
    }
...
//    }
    ...
    [mDict setObject:SafeStr(self.liveView.cid) forKey:@"video_classified"];
