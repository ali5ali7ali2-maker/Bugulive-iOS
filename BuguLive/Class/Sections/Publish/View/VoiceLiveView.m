...
@interface VoiceLiveView()<AVCaptureVideoDataOutputSampleBufferDelegate, UITextViewDelegate,FWTLiveBeautyViewDelegate,TXVideoCustomProcessDelegate,UIGestureRecognizerDelegate,BeautySettingPanelDelegate>
{
...
//@property (nonatomic, strong) TXLivePush        *pusher;
@property (nonatomic, strong) UIImageView          *preViewContainer;
...
@property (nonatomic, strong) CustomTagCollectionView *tagCollectionViewPeople;

//@property(nonatomic, strong) JCTagListView *tagListViewPeople;
@property(nonatomic, strong) UIView *s2;
@property(nonatomic, strong) UIView *s3;
@property(nonatomic, strong) RoomPasswordView *passwordView;
@end

@implementation VoiceLiveView
...
//        self.topView = [[VoiceLiveTopView alloc]initWithFrame:CGRectMake(0, 10, self.width, 0.01)];
...
//        self.autoresizingMask =UIViewAutoresizingFlexibleHeight;
        ...
        labRoomTag.textColor = [UIColor qmui_colorWithHexString:@"#D074FF"];
        [self.s2 addSubview:labRoomTag];
        
        self.tagListView = [[JCTagListView alloc] initWithFrame:CGRectMake(kRealValue(20), kRealValue(40), self.width - kRealValue(20), 0)];
        ...
        for (int i = 0; i < [GlobalVariables sharedInstance].appModel.video_classified.count; i++)
        {
            VideoClassifiedModel * model = [GlobalVariables sharedInstance].appModel.video_classified[i];
        ...
        self.cid = [NSString stringWithFormat:@"%ld",(long)((VideoClassifiedModel *)[GlobalVariables sharedInstance].appModel.video_classified.firstObject).classified_id];

        self.tagListView.selectedTags = @[video_classified.firstObject];
        
        [self.tagListView didSelectItem:^(NSInteger index) {
            VideoClassifiedModel * model = [GlobalVariables sharedInstance].appModel.video_classified[index];
        ...
        // 创建s3 view
        self.s3 = [[UIView alloc] initWithFrame:CGRectMake(self.s2.left, self.s2.bottom + kRealValue(10), self.width-kRealValue(20), 120)];
        [self.middleView addSubview:self.s3];
        ViewRadius(self.s3, kRealValue(5));

        self.s3.backgroundColor = [[UIColor colorWithHexString:@"#000000"] colorWithAlphaComponent:0.15];
        ...
        [_passwordView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.equalTo(self.s3.mas_bottom).offset(20);
        ...
        // 添加标题
        UILabel *labPeople = [[UILabel alloc] initWithFrame:CGRectMake(kRealValue(15), kRealValue(20.5), 100, kRealValue(16))];
        labPeople.text  = @"Mic mode";
        labPeople.font = [UIFont systemFontOfSize:15];
        labPeople.textColor = [UIColor qmui_colorWithHexString:@"#D074FF"];
        [self.s3 addSubview:labPeople];

        // 创建tagListView
        // 创建 tagCollectionViewPeople
                self.tagCollectionViewPeople = [[CustomTagCollectionView alloc] initWithFrame:CGRectMake(0, kRealValue(40), self.s3.width, 300)];
                [self.s3 addSubview:self.tagCollectionViewPeople];
                
                self.tagCollectionViewPeople.tags = @[ASLocalizedString(@"5"), ASLocalizedString(@"10"),ASLocalizedString(@"15"),ASLocalizedString(@"20")];
                
                __weak typeof(self) weakSelf = self;
                self.tagCollectionViewPeople.didSelectItemBlock = ^(NSInteger index) {
                    NSLog(@"%@", @(index));
                    if(index == 0) {
                        weakSelf.people_count = @"5";
                    } else if(index == 1) {
                        weakSelf.people_count = @"10";
                    } else if(index == 2) {
                        weakSelf.people_count = @"15";
                    } else {
                        weakSelf.people_count = @"20";
                    }
                };

                [self.tagCollectionViewPeople reloadData];
                self.tagCollectionViewPeople.height = 300;
                self.s3.height = CGRectGetMaxY(self.tagCollectionViewPeople.frame) + kRealValue(10);

                if(self.s3.height < kRealValue(120)) {
                    self.s3.height = kRealValue(120);
                }
        
        
        UILabel *lab = [[UILabel alloc]initWithFrame:CGRectMake(self.width / 2  - 50, self.selectedImageView.bottom + 15, 100, 20)];
        lab.text = ASLocalizedString(@"选择封面");
        ...
        self.beautyBtn.hidden = YES;
//        self.textView.top = lab.bottom + 30;
...
//        [self.startButton setBackgroundImage:[UIImage imageWithColor:[UIColor qmui_colorWithHexString:@"#AE2CF1"]] forState:UIControlStateNormal];
        ...
        gradientLayer.colors = @[
            (id)[UIColor qmui_colorWithHexString:@"#AAD2F0"].CGColor,
            (id)[UIColor qmui_colorWithHexString:@"#6280BF"].CGColor
        ];

        [self.startButton.layer insertSublayer:gradientLayer atIndex:0];
        self.startButton.layer.cornerRadius = kRealValue(22.5);

        [self addSubview:self.startButton];
        
        [self.startButton mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.passwordView.mas_bottom).offset(kRealValue(20));
        ...
        [self setTxPreview];
        [self resetViewWithTitleTop:kRealValue(23) + kStatusBarHeight];
...
//        [_voiceBtn setImage:[UIImage imageNamed:@"chatparty222"] forState:UIControlStateNormal];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            // 需要延迟执行的代码
            self.contentSize = CGSizeMake(kScreenW, kScreenH+600);
...
//    _pusher = [[TXLivePush alloc] initWithConfig:_txLivePushonfig];
    ...
    // 初始化视频父视图
//    _preViewContainer = [[UIImageView alloc] initWithFrame:self.bounds];
...
- (void)otherPushBtnAction:(UIButton *)sender{
    ...
    [self.startButton setTitle:@"创建外设直播" forState:UIControlStateNormal];
}
...
- (UIView *)lineView{
    if (!_lineView) {
        ...
        _lineView.centerX = self.voiceBtn.centerX;
    }
    return _lineView;
}
