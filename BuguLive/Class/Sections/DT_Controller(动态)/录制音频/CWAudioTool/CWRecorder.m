...
//    NSLog(ASLocalizedString(@"创建中..."));
    ...
    if (![self initAudioRecorder]) { // 初始化录音机
        NSLog(ASLocalizedString(@"录音机创建失败..."));
        if (self.delegate && [self.delegate respondsToSelector:@selector(recorderFailed:)]) {
            [self.delegate recorderFailed:ASLocalizedString(@"录音器创建失败")];
        }
        return;
    };
//    NSLog(ASLocalizedString(@"创建完成..."));
    [self micPhonePermissions:^(BOOL ishave) {
        if (ishave) {
            [self startRecording];
        }else {
            [self showPermissionsAlert];
            NSLog(ASLocalizedString(@"麦克风未开启权限"));
        }
    }];
}
...
//    NSLog(@"startRecording...");
    ...
    NSLog(ASLocalizedString(@"初始化..."));
    if (![self.audioRecorder prepareToRecord]) {
        NSLog(ASLocalizedString(@"初始化录音机失败"));
        if (self.delegate && [self.delegate respondsToSelector:@selector(recorderFailed:)]) {
            [self.delegate recorderFailed:ASLocalizedString(@"录音器初始化失败")];
        }
        return;
    }
...
- (void)showPermissionsAlert {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:ASLocalizedString(@"无法录音")message:ASLocalizedString(@"请在“设置-隐私-麦克风”中允许访问麦克风。")preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *action = [UIAlertAction actionWithTitle:ASLocalizedString(@"确认")style:UIAlertActionStyleDefault handler:nil];
