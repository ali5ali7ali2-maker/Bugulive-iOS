...
//    [manager cacheKeyForURL:<#(nullable NSURL *)#>]
...
- (void)showOriImage{
    ...
    tempView.image = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:[self highQualityImageURLForIndex:self.currentImageIndex].absoluteString];
...
- (void)imageViewLongPressed:(UILongPressGestureRecognizer *)gesture{
    if(gesture.state == UIGestureRecognizerStateBegan){
        ...
        YHActionSheet *aSheet = [[YHActionSheet alloc] initWithCancelTitle:ASLocalizedString(@"取消")otherTitles:@[ASLocalizedString(@"保存图片")]];
        [aSheet dismissForCompletionHandle:^(NSInteger clickedIndex, BOOL isCancel) {
            if (!isCancel) {
                switch (clickedIndex) {
                    case 0:
                        NSLog(ASLocalizedString(@"保存图片"));
                        [weakSelf saveImage];
                        break;
...
- (void)setupImageOfImageViewForIndex:(NSInteger)index
{
    ...
    if ([self hasHighQImage]) {
        ...
        imageView.image = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:[self highQualityImageURLForIndex:self.currentImageIndex].absoluteString];
    }
    else{
