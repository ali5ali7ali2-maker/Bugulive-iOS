...
- (void)setMHaveHeader:(BOOL)mHaveHeader
{
    ...
    if( _mHaveHeader )
    {
        [BGMJRefreshManager refresh:self.mTableView target:self headerRereshAction:@selector(headerStartRefresh) shouldHeaderBeginRefresh:NO footerRereshAction:nil];
    }
}
...
- (void)setMHaveFooter:(BOOL)mHaveFooter
{
    ...
    if( _mHaveFooter )
    {
        [BGMJRefreshManager refresh:self.mTableView target:self headerRereshAction:nil footerRereshAction:@selector(footerStartRefresh)];
    }
}
