...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        _diamondsLabel.text = [NSString stringWithFormat:ASLocalizedString(@"%@余额:1239398"),self.BuguLive.appModel.diamond_name];
        ...
        _rateLabel.text = ASLocalizedString(@"兑换比例:20");
...
//        _textField.borderStyle = UITextBorderStyleRoundedRect;
        _textField.placeholder = ASLocalizedString(@"请输入金额");
        ...
        _gameCoinLabel.text = ASLocalizedString(@"0游戏币");
...
//        [self addSubview:_verticalLineView];
        ...
        [_cancaleBtn setTitle:ASLocalizedString(@"取消")forState:UIControlStateNormal];
        ...
        [_makeSureBtn setTitle:ASLocalizedString(@"确定")forState:UIControlStateNormal];
...
- (void)changeGameCoinLabel
{
    ...
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"单次兑换超过上限")];
        return;
    }
    NSInteger  rateCoin = [_textField.text integerValue];
    rateCoin = rateCoin*[self.model.exchange_rate floatValue];
    _gameCoinLabel.text = [NSString stringWithFormat:ASLocalizedString(@"%zd游戏币"),rateCoin];
}
...
- (void)exchangeCoin
{
    ...
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请输入有效金额")];
        return;
    }
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
     {
         ...
         if ([responseJson toInt:@"status"]==1)
         {
             ...
             self.diamondsLabel.text = [NSString stringWithFormat:ASLocalizedString(@"%@余额:%zd"),self.BuguLive.appModel.diamond_name,self.model.diamonds];
             [UIView animateWithDuration:0.5 animations:^{
                 self.frame =CGRectMake(10,kScreenH, kScreenW-20, 230);
             } completion:^(BOOL finished) {
                 self.hidden = YES;
                 self.textField.text = nil;
                 self.gameCoinLabel.text = ASLocalizedString(@"0游戏币");
             }];
         }
...
{
    _model = model;
    self.rateLabel.text = [NSString stringWithFormat:ASLocalizedString(@"兑换比例: %@"),model.exchange_rate];
    self.diamondsLabel.text = [NSString stringWithFormat:ASLocalizedString(@"%@余额:%zd"),self.BuguLive.appModel.diamond_name,model.diamonds];
}
