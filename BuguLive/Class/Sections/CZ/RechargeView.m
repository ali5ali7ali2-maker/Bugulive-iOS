...
- (instancetype)initWithFrame:(CGRect)frame andUIViewController:(UIViewController *)viewController
{
    if (self = [super initWithFrame:frame])
    {
        ...
        //_lineView.backgroundColor = [BGUtils colorWithHexString:@"4dffffff"];
        ...
        [_exchangeBtn setTitle:ASLocalizedString(@"兑换")forState:UIControlStateNormal];
        //[_exchangeBtn setTitleColor:[BGUtils colorWithHexString:@"4dffffff"] forState:UIControlStateNormal];
        ...
        if (_BuguLive.appModel.open_game_module == 0 || _BuguLive.appModel.open_diamond_game_module == 1)
        {
        ...
        _rechargeWayView = [[RechargeWayView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_selectLineView.frame) - kRealValue(5), self.width, 40)];
        _rechargeWayView.delegate = self;
        [self addSubview:_rechargeWayView];
        
        self.agreeBtn.frame = CGRectMake(kRealValue(20), _rechargeWayView.bottom + kRealValue(5), self.width, kRealValue(30));
        [self addSubview:self.agreeBtn];
        [self addSubview:self.selectAgreeBtn];
        
        _separateView = [[UIView alloc] initWithFrame:CGRectMake(10, _rechargeWayView.bottom + 15, self.width-20, 0.5)];
...
- (void)reloadView
{
    NSString * str = [NSString stringWithFormat:ASLocalizedString(@"余额：%zd"),self.model.diamonds];
...
- (void)clickBtn:(UIButton *) button
{
    ...
    {
        if (_delegate && [_delegate respondsToSelector:@selector(closeRechargeWithRechargeView:)])
        {
            [_delegate closeRechargeWithRechargeView:self];
        }
...
- (void)goToPayWithPayModel:(PayMoneyModel *)model
{
    if (_hadClick)
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"正在提交中")];
        return;
    }
    else
    {
...
- (void)payRequestWithModel:(PayMoneyModel *)payModel
{
    ...
    [[BGHUDHelper sharedInstance] syncLoading:ASLocalizedString(@"支付请求中,请稍后")inView:self];
...
//                [BGInterface start:_viewController withParams:param  withDelegate:self];
                //[BGInterface start:_viewController withParams:param withType:model.withType withDelegate:self];
                // 凡伟支付 end
            ...
            else if ([payDic toInt:@"is_wap"] == 1)
            {
                ...
                {
                    BGMainWebViewController *vc = [BGMainWebViewController webControlerWithUrlStr:[payDic stringForKey:@"url"] isShowIndicator:YES isShowNavBar:YES];
