...
- (instancetype)initWithFrame:(CGRect)frame andUIViewController:(UIViewController *)viewController
{
    if (self = [super initWithFrame:frame])
    {
        ...
        _diamondsLabel.text = [NSString stringWithFormat:@"0%@",self.BuguLive.appModel.diamond_name];
...
{
    _model = model;
    _rateLabel.text = [NSString stringWithFormat:ASLocalizedString(@"兑换比例:%@"),model.rate];
}
...
- (void)clickBtn:(UIButton *)button
{
    if (button==_makeSureBtn)
    {
        ...
        {
            [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请输入有效的兑换金额")];
            return;
        }
...
- (void)changeDiamondsLabel
{
    ...
    _diamondsLabel.text = [NSString stringWithFormat:@"%zd%@",rateCoin,self.BuguLive.appModel.diamond_name];
}
...
- (void)disChangeText
{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        self.diamondsLabel.text = [NSString stringWithFormat:@"0%@",self.BuguLive.appModel.diamond_name];
