...
static float const      kSegmentTitleFont       = 15;   // segmentedControl字体大小
...
@property (nonatomic, strong) NewestViewController  *news_hotViewController;  // 最新
...
@property (nonatomic, assign) BOOL                                      hasAddedVideoClassifiedObserver;
...
- (void)initFWUI
{
    ...
    self.BuguLive.appNavigationBarHeight = self.navigationController.navigationBar.frame.size.height;
    self.BuguLive.appTabBarHeight = self.tabBarController.tabBar.frame.size.height;
    
    _startPage = kSelectedSegmentIndex;
    
    _videoVCMutableArray = [NSMutableArray array];
    _oldSocietyListName = self.BuguLive.appModel.society_list_name;
    ...
    self.segmentedControl = [[HMSegmentedControl alloc] initWithFrame:CGRectMake(60, kStatusBarHeight, viewWidth - 60 - 80 , kSegmentedHeight)];
...
//    [self.navigationController.navigationBar addSubview:_segmentedControl];
    ...
    CGFloat scrollViewHeight = MAX(0.0, viewHeight - kSegmentedHeight - kTabBarHeight - kStatusBarHeight);
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, _segmentedControl.bottom, viewWidth, scrollViewHeight)];
    ...
    if ([self.itemTitleMutableArray containsObject:kFocusOnStr])
    {
        ...
        _focusOnViewController.collectionViewFrame = CGRectMake(0, 0, viewWidth, _scrollView.frame.size.height);
    ...
    if ([self.itemTitleMutableArray containsObject:kHotStr])
    {
        ...
        _news_hotViewController = [[NewestViewController alloc]init];
        _news_hotViewController.types = @"1";
        _news_hotViewController.collectionViewFrame = CGRectMake(0, 0, viewWidth, _scrollView.bounds.size.height);
        _news_hotViewController.view.frame = CGRectMake(viewWidth * [self.itemTitleMutableArray indexOfObject:kHotStr], 0, viewWidth, _scrollView.bounds.size.height);
        _news_hotViewController.delegate = self;
        [_scrollView addSubview:_news_hotViewController.view];
...
//        [_scrollView addSubview:_hotViewController.view];
    ...
    if ([self.itemTitleMutableArray containsObject:kNearbyStr])
    {
        ...
        _livingListViewController.types = @"2";
    ...
    if (!self.hasAddedVideoClassifiedObserver) {
        [self.BuguLive addObserver:self forKeyPath:@"appModel.video_classified" options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:nil];
        self.hasAddedVideoClassifiedObserver = YES;
    }
...
#else
    // 左上角按钮
    UIButton *leftButton = [[UIButton alloc]initWithFrame:CGRectMake(20, kStatusBarHeight, 35, kNavigationBarHeight)];
...
//    self.navigationItem.leftBarButtonItem = leftBarButtonItem;
    ...
    // 右上角按钮
    UIView *rightViewContrainer = [[UIView alloc] initWithFrame:CGRectMake(kScreenW - 70 - 10, kStatusBarHeight, 70, kNavigationBarHeight)];
    ...
    [self.view addSubview:rightViewContrainer];
    [self.view addSubview:leftButton];
...
- (void)loadClassifiedVC
{
    ...
    // 动态添加、删除公会
    if (self.BuguLive.appModel.open_society_module.intValue == 1 && ![BGUtils isBlankString:self.BuguLive.appModel.society_list_name])
    {
        _societyListName = self.BuguLive.appModel.society_list_name;
    ...
    else if (self.BuguLive.appModel.open_society_module.intValue == 0 && [self.itemTitleMutableArray containsObject:_societyListName])
    {
    ...
    if (![self.BuguLive.appModel.video_classified isEqual:self.classifiedModelMutableArray])
    {
...
- (void)updateClassiFiedVC
{
    ...
    @synchronized (self)
    {
        ...
        self.classifiedModelMutableArray = self.BuguLive.appModel.video_classified;
        ...
        for (NSInteger i = 0; i < self.classifiedModelMutableArray.count; ++i)
        {
            ...
            VideoClassifiedModel * model = [self.BuguLive.appModel.video_classified objectAtIndex:i];
...
- (void)refreshClassifiedVC:(NSInteger)page
{
    ...
    NSString *tmpStr = [BGUtils isBlankString:self.news_hotViewController.areaString] ? kHotStr : self.news_hotViewController.areaString;
...
- (void)clickRightHeadBtn:(UIButton *)btn
{
    if (btn.tag == 1)
    {
        if (!self.chooseAreaView)
        {
            self.chooseAreaView = [[SChooseAreaView alloc]initWithFrame:CGRectMake(0, kScreenH, kScreenW, kScreenH-kStatusBarHeight-kNavigationBarHeight) andChooseType:[self.news_hotViewController.sexString intValue] andAreaStr:self.news_hotViewController.areaString];
            FWWeakify(self)
            [self.chooseAreaView setAreaBlock:^(NSString *areStr, int sexType){
                ...
                if (areStr) {
                    ...
                    self.segmentedControl.sectionTitles = self.itemTitleMutableArray;
                    self.news_hotViewController.areaString = areStr;
                    self.news_hotViewController.sexString  = [NSString stringWithFormat:@"%d",sexType];
                    [self.news_hotViewController loadDataFromNet:1];
                    [self removeChooseView];
                }
            }];
...
- (void)setupTimer
{
    @synchronized (self)
    {
        if (_refreshHomeTimer && [_refreshHomeTimer isValid]) {
            return;
        }
        [_refreshHomeTimer invalidate];
        _refreshHomeTimer = [NSTimer timerWithTimeInterval:kRefreshWithNewaTimeInterval target:self selector:@selector(refreshOfTime) userInfo:nil repeats:YES];
        [[NSRunLoop mainRunLoop] addTimer:_refreshHomeTimer forMode:NSRunLoopCommonModes];
    }
}
...
{
    // 防止短时间内重复刷新
    if (self.isRefreshing && [[[BGTabBarController sharedInstance] selectedViewController].childViewControllers[0] isKindOfClass:[HMHomeViewController class]])
    {
    ...
    NSLog(ASLocalizedString(@"============调起首页刷新机制时间点：%@"),[dateformatter stringFromDate:[NSDate date]]);
    ...
    if (_news_hotViewController)
    {
        [_news_hotViewController loadDataFromNet:1];
    }
    ...
    if (self.BuguLive.appModel.open_society_module.intValue == 1)
    {
...
- (void)pushSocietyDetail:(SociatyListModel *)listModel
{
    ...
    [[AppDelegate sharedAppDelegate] pushViewController:tianTuanVC animated:YES];
}
...
{
    if (object == self.BuguLive && [keyPath isEqualToString:@"appModel.video_classified"]) {
        [self updateClassiFiedVC];
        return;
    }
...
- (void)dealloc
{
    if (self.hasAddedVideoClassifiedObserver) {
        @try {
            [self.BuguLive removeObserver:self forKeyPath:@"appModel.video_classified" context:nil];
        } @catch (__unused NSException *exception) {
        } @finally {
            self.hasAddedVideoClassifiedObserver = NO;
        }
    }
