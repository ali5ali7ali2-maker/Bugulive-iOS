...
- (void)createTableView
{
    if (!_listTableView)
    {
        ...
        {
            _listTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, kScreenW , kScreenH-kStatusBarHeight-44 -44 )];
            if (isIPhoneX())
            {
                _listTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH-kTopHeight-44 -self.bottomView.height - MG_BOTTOM_MARGIN - 44)];
            }
        }
        else
        {
            _listTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, kScreenW , kScreenH-kStatusBarHeight-44 -44-49)];
            if (isIPhoneX())
            {
                _listTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH-kStatusBarHeight*2-44 -44-40)];
            }
        }
        ...
        self.leadheadView =  [[SLeaderHeadView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, 265)];
        FWWeakify(self)
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
     {
         ...
         if ([responseJson toInt:@"status"] == 1)
         {
             ...
             [self getRankListNum:[NSString stringWithFormat:@"%d",[responseJson toInt:@"my_ranking"]]];
         }
         [BGMJRefreshManager endRefresh:self.listTableView];
...
-(void)getRankListNum:(NSString *)rankNum{
...
-(ContributionLsitBottomView *)bottomView{
    if (!_bottomView) {
        _bottomView = [[ContributionLsitBottomView alloc]initWithFrame:CGRectMake(0, _listTableView.height - kRealValue(48), kScreenW, kRealValue(48))];
    }
    return _bottomView;
}
