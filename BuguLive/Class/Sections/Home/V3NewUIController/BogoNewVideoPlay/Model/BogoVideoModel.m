...
@implementation BogoVideoModel

+ (NSArray<BogoVideoModel *> *)testItems {
    NSMutableArray<BogoVideoModel *> *m = [[NSMutableArray alloc] initWithCapacity:20];
    for ( int i = 0 ; i < 20 ; ++ i ) {
        [m addObject:[BogoVideoModel testItem]];
    }
    return m;
}

+ (NSArray<BogoVideoModel *> *)testItemsWithCount:(NSInteger)count {
    NSMutableArray<BogoVideoModel *> *m = [[NSMutableArray alloc] initWithCapacity:count];
    for ( int i = 0 ; i < count ; ++ i ) {
        [m addObject:[BogoVideoModel new]];
    }
    return m;
}

+ (instancetype)testItem {
    BogoVideoModel *item = [BogoVideoModel new];
