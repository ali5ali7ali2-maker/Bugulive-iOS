//
//  BogoRechargePayTypeCollection.m
//  BuguLive
//
...
@implementation BogoRechargePayTypeCollection
...
//    layout.headerReferenceSize=CGSizeMake(kScreenW, kRealValue(166));//头视图的大小
    ...
    [_collectionView registerNib:[UINib nibWithNibName:@"BogoRechargePayTypeCell" bundle:nil] forCellWithReuseIdentifier:@"BogoRechargePayTypeCell"];
...
- (void)setModel:(AccountRechargeModel *)model{
    _model = model;
    self.listArr = [NSMutableArray arrayWithArray:model.pay_list];
    if (self.listArr.count > 0) {
        PayTypeModel *first = self.listArr.firstObject;
        first.isSelect = YES;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BogoRechargePayTypeCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"BogoRechargePayTypeCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PayTypeModel *selectModel = [self.listArr objectAtIndex:indexPath.row];
    
    selectModel.isSelect = YES;
    
    for (int i = 0; i < self.listArr.count; i++) {
        PayTypeModel *model = self.listArr[i];

        model.isSelect = NO;
        if (selectModel.payWayID == model.payWayID) {
            selectModel.isSelect = YES;
...
-(UILabel *)titleL{
    if (!_titleL) {
        ...
        _titleL.text = ASLocalizedString(@"请选择支付方式");
