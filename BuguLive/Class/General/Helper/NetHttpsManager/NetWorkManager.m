...
+ (NSString *)getUrlStr:(NSMutableDictionary *)parmDict
{
    ...
    NSString *urlString = BuguLive.currentDoMianUrlStr;
    
    if (BuguLive.appModel.api_link)
    {
        if ([BuguLive.appModel.api_link count])
        {
            for (ApiLinkModel *apiLinkModel in BuguLive.appModel.api_link)
            {
...
+(void)tryAddLangParamDict:(NSMutableDictionary *)paramDict
{
    if(![[paramDict allKeys] containsObject:@"language"])
    {
//        [[NSUserDefaults standardUserDefaults] setObject:@"zh-Hans" forKey:KAppLanguage];
        ...
        {
            [paramDict setObject:@"zh-cn" forKey:@"language"];
        }
        else
        {
            [paramDict setObject:@"zh-en" forKey:@"language"];
        }
        
    }
}

+(NSMutableDictionary *)tryAddLangParamDict2:(NSDictionary *)paramDict
{
    NSMutableDictionary *tmp = [NSMutableDictionary dictionaryWithDictionary:paramDict];
    
    if(![[tmp allKeys] containsObject:@"language"])
    {
//        [[NSUserDefaults standardUserDefaults] setObject:@"zh-Hans" forKey:KAppLanguage];
        ...
        if([lang isEqualToString:@"zh-Hans"])
        {
            [tmp setObject:@"zh-cn" forKey:@"language"];
        }
        else
        {
            [tmp setObject:@"zh-en" forKey:@"language"];
        }
...
+ (NSMutableDictionary *)getLocalParm:(NSMutableDictionary *)parmDict url:(NSString *)urlStr
{
    ...
    [self tryAddLangParamDict:parmDict];
    GlobalVariables *BuguLive = [GlobalVariables sharedInstance];
    
    if([GlobalVariables sharedInstance].openinstallDic != nil)
    {
//        [parmDict setDictionary:[GlobalVariables sharedInstance].openinstallDic];
         [parmDict addEntriesFromDictionary:[GlobalVariables sharedInstance].openinstallDic];
    }
    ...
    //    [parmDict setObject:[NSString stringWithFormat:@"%f",kScreenW] forKey:@"kScreenW"];
    //    [parmDict setObject:[NSString stringWithFormat:@"%f",kScreenHeight] forKey:@"kScreenH"];
    if (BuguLive.longitude && BuguLive.latitude)
    {
        [parmDict setObject:[NSString stringWithFormat:@"%f",BuguLive.longitude] forKey:@"xpoint"]; // 经度
        [parmDict setObject:[NSString stringWithFormat:@"%f",BuguLive.latitude] forKey:@"ypoint"];  // 经度
        [parmDict setObject:[NSString stringWithFormat:@"%f",BuguLive.longitude] forKey:@"lat"]; // 经度
        [parmDict setObject:[NSString stringWithFormat:@"%f",BuguLive.latitude] forKey:@"lng"];  // 经度
    }
    ...
    {
//        if (BuguLive.aesKeyStr.length != 16)
//        {
//            [FanweMessage alert:ASLocalizedString(@"您的AES加密密码长度不是16位！")];
//        }
        // AES加密
        NSData *data = [NSJSONSerialization dataWithJSONObject:parmDict options:NSJSONWritingPrettyPrinted error:nil];
        if (data)
        {
            NSString *parmStr = [data AES256EncryptWithKey:BuguLive.aesKeyStr];
            if (![BGUtils isBlankString:parmStr])
            {
...
//                [parmDict setObject:@"lib" forKey:@"i_type"];
...
+ (void)asyncPostWithFileUrl:(NSURL *)fileUrlStr url:(NSString *)urlStr paramDict:(NSMutableDictionary *)paramDict successBlock:(SuccessBlock2)successBlock failureBlock:(FailureBlock2)failureBlock
{
    ...
    {
        NSLog(ASLocalizedString(@"请检查当前网络"));
    }
    else
    {
        ...
        [manager POST:urlStr parameters:paramDict headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData){
...
+ (void)asyncGetWithUrl:(NSString *)urlStr headers:(NSMutableDictionary *)headers successBlock:(SuccessBlock2)successBlock failureBlock:(FailureBlock2)failureBlock
{
    ...
    else
    {
        ...
        [manager GET:urlStr parameters:[self getLocalParm:[NSMutableDictionary dictionary] url:urlStr] headers:nil progress:^(NSProgress * _Nonnull downloadProgress) {
...
+ (void)doResult:(id)responseObject url:(NSString *)urlStr paramDict:(NSMutableDictionary *)paramDict successBlock:(SuccessBlock)successBlock successBlock2:(SuccessBlock2)successBlock2 failureBlock:(FailureBlock)failureBlock failureBlock2:(FailureBlock2)failureBlock2
{
    ...
    if ([IsNeedAES isEqualToString:@"YES"] && [responseObject isKindOfClass:[NSData class]])
    {
        ...
        if (tmpResposeJson && [tmpResposeJson isKindOfClass:[NSDictionary class]])
        {
            ...
            if (![BGUtils isBlankString:tmpJsonStr])
            {
                // base64解密
                NSData *decodeData = [GTMBase64 decodeString:tmpJsonStr];
                // AES解密
                NSString *resultStr = [decodeData AES256DecryptWithKey:AppAESKey];

//                NSString *resultStr = [decodeData AES256DecryptWithKey:BuguLive.aesKeyStr];
                
                // 如果解密失败，重新请求aeskey
                if ([BGUtils isBlankString:resultStr])
                {
                    [[BGIMLoginManager sharedInstance] obtainAesKeyFromFullGroup:nil error:nil];
                    
                    NSLog(ASLocalizedString(@"===================：解密失败，请检查AES加密的KEY等！"));
                    if (failureBlock2)
                    {
                        failureBlock2(nil, [AppNetWorkModel manager]);
                    }
            ...
            {
//                responseObject = tmpJsonStr;
                [[BGIMLoginManager sharedInstance] obtainAesKeyFromFullGroup:nil error:nil];
            }
        }
        else
        {
            NSLog(ASLocalizedString(@"===================：获取加密串失败！"));
        }
    }
    ...
    if (resposeJson && [resposeJson isKindOfClass:[NSDictionary class]])
    {
        if([resposeJson count])
        {
            ...
            BGBaseModel *tmpModel = [BGBaseModel mj_objectWithKeyValues:resposeJson];
            if (![BGUtils isBlankString:tmpModel.errorStr])
            {
                if (tmpModel.status != 1 && ![tmpModel.act isEqualToString:@"firebase_login"])
                {
                    if (![[paramDict objectForKey:@"act"] isEqualToString:@"request_end_pk"] && ![[paramDict objectForKey:@"act"] isEqualToString:@"mix_stream"]) {
                        [FanweMessage alertHUD:tmpModel.errorStr];
                    }
                   
                    NSLog(ASLocalizedString(@"===================：错误信息：接口名为 ctl=%@,act=%@，弹出错误信息：%@"),tmpModel.ctl,tmpModel.act,tmpModel.errorStr);
                }
                else
                {
                    NSLog(ASLocalizedString(@"===================：正确信息：接口名为 ctl=%@,act=%@，弹出正确信息：%@"),tmpModel.ctl,tmpModel.act,tmpModel.errorStr);
                }
            }
            ...
            {
                if ([[resposeJson toString:@"init_version"] longLongValue] > BuguLive.appModel.init_version)
                {
        ...
        {
            [self updateErrorToServiceWithUrl:urlStr paramDict:paramDict errorString:ASLocalizedString(@"返回的数据为空")];
            if (failureBlock2)
            {
                failureBlock2(nil, [AppNetWorkModel manager]);
            }
...
+ (void)updateErrorToServiceWithUrl:(NSString *)urlStr paramDict:(NSMutableDictionary *)paramDict errorString:(NSString *)errorString
{
    ...
    else
    {
        ...
        [manager POST:urlStr parameters:errorDict headers:nil progress:^(NSProgress * _Nonnull uploadProgress) {
