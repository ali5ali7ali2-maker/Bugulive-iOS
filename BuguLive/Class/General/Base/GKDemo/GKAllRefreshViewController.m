...
- (void)viewDidLoad {
    ...
    self.pageScrollView.mainTableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        {
            VoiceListViewController *currentListVC = self.childVCs[self.segmentView.selectedIndex];
            [currentListVC headerReresh];
        }

    }];
    
    self.pageScrollView.mainTableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        // 取出当前显示的listView
        
        if(self.childVCs.count)
        {
            [self.pageScrollView.mainTableView.mj_footer endRefreshing];
            VoiceListViewController *currentListVC = self.childVCs[self.segmentView.selectedIndex];
            [currentListVC footerRereshing];
            [self.pageScrollView reloadData];
        }
