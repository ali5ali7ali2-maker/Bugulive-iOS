...
#define IsArgInvalid(curView, brotherView) NSAssert((self.superview == brotherView.superview) && !(CGRectEqualToRect(brotherView.frame, CGRectZero)), ASLocalizedString(@"UIView (CustomAutoLayout)参数出错"))
#define IsSuperViewInvalid(curView) NSAssert((self.superview != nil), ASLocalizedString(@"UIView (CustomAutoLayout)父控件没有设置参数出错"))
