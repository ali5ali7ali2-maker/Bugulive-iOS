...
- (void)createBtn:(NSArray *)titlesArr addScroll:(UIScrollView*)scroll startX:(CGFloat)start_x start_index:(NSInteger)start_index {
    ...
    for (NSInteger i = start_index; i < titlesArr.count; i ++) {
        ...
        if (titles) {
            ...
            if (self.btnBgImg) {
                UIImage *backgroundImage = [UIImage imageNamed:self.btnBeforeBgImg];
                UIEdgeInsets capInsets = UIEdgeInsetsMake(backgroundImage.size.height / 2, backgroundImage.size.width / 2, backgroundImage.size.height / 2, backgroundImage.size.width / 2);
                UIImage *resizableBackgroundImage = [backgroundImage resizableImageWithCapInsets:capInsets resizingMode:UIImageResizingModeStretch];
                [button setBackgroundImage:resizableBackgroundImage forState:UIControlStateNormal];
                
//                [button setBackgroundImage:[UIImage imageNamed:self.btnBeforeBgImg] forState:UIControlStateNormal];
            }
            ViewRadius(button,  MIN(_slideCorner, _slideHeight/2));
            
            //把默认选中设置为btnBgImg
            if (currentIndex == i) {
                if (self.btnBgImg) {
                    UIImage *backgroundImage = [UIImage imageNamed:self.btnBgImg];
                    UIEdgeInsets capInsets = UIEdgeInsetsMake(backgroundImage.size.height / 2, backgroundImage.size.width / 2, backgroundImage.size.height / 2, backgroundImage.size.width / 2);
                    UIImage *resizableBackgroundImage = [backgroundImage resizableImageWithCapInsets:capInsets resizingMode:UIImageResizingModeStretch];
                    [button setBackgroundImage:resizableBackgroundImage forState:UIControlStateNormal];
...
//    }
    ...
    if (self.btnBgImg) {
        ...
        UIEdgeInsets capInsets = UIEdgeInsetsMake(backgroundImage.size.height / 2, backgroundImage.size.width / 2, backgroundImage.size.height / 2, backgroundImage.size.width / 2);
        UIImage *resizableBackgroundImage = [backgroundImage resizableImageWithCapInsets:capInsets resizingMode:UIImageResizingModeStretch];
        [select_btn setBackgroundImage:resizableBackgroundImage forState:UIControlStateNormal];
