...
- (void)alertResult{
    ...
    label.text = self.isBlack?ASLocalizedString(@"白方胜"):ASLocalizedString(@"黑方胜");
    [label sizeToFit];
...
- (void)backOneStep:(UIButton *)sender{
    ...
    if (self.lastKey == nil) {
        ...
        tip.center = CGPointMake(self.width * 0.5, self.height * 0.5);
        UILabel * label = [[UILabel alloc]init];
        label.text = self.chessmanDict.count > 0 ? ASLocalizedString(@"只能悔一步棋!!!"): ASLocalizedString(@"请先落子!!!");
