...
- (void)confirmAuction
{
    ...
    else
    {
        ...
        {
            [[BGHUDHelper sharedInstance]tipMessage:ASLocalizedString(@"请选择玩家")];
        }
        else
        {
            [[BGHUDHelper sharedInstance]tipMessage:ASLocalizedString(@"暂时没有玩家上庄")];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        cell.coinLabel.text = [NSString stringWithFormat:@"%@%@",model.coin,self.BuguLive.appModel.diamond_name];
    }
    else
    {
        cell.coinLabel.text = [NSString stringWithFormat:ASLocalizedString(@"%@游戏币"),model.coin];
    }
