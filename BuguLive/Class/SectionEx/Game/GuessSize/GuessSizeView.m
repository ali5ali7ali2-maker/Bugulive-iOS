...
-(void)loadBetDesDataWithBetLabel:(UILabel *)betLabel andBetNumber:(id)betNumber
{
    ...
    if ([betNumber integerValue]>=10000) {
        ...
        betLabel.text = [NSString stringWithFormat:ASLocalizedString(@"%.1f万"),bet];
    }
    else
    {
...
-(void)loadUserBetDesDataWithLabel:(UILabel *)userBetLabel andUserBetNumber:(id)userBetNumber
{
    ...
    if ([userBetNumber integerValue]>=10000) {
        ...
        userBetLabel.text = [NSString stringWithFormat:ASLocalizedString(@"%.1f万"),userBet];
    }
    else
    {
...
- (void)postBetMoneyStr:(NSString *)str withTag:(NSInteger)tag
{
    ...
    [_httpManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson) {
        ...
        if ([responseJson toInt:@"status"] == 1)
        {
            ...
            if (_BuguLive.appModel.open_diamond_game_module == 1)
            {
...
- (void)gameOver
{
    ...
    [_httpManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson) {
        ...
        _canCloseGame = YES;
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"关闭游戏失败")];
        NSLog(@"---%@",error);
    }];
}
...
- (void)coinMove:(UITapGestureRecognizer *)tap
{
    if (!_isHost && _canBet)
    {
        ...
        {
            [SVProgressHUD showErrorWithStatus:ASLocalizedString(@"金额不足，请充值")];
        }
    }
}
...
- (void)selectAmount:(UIButton *)sender
{
    if (_isHost) {
        ...
        else if (sender.tag == 206)
        {
            ...
            {
                FWWeakify(self)
                [FanweMessage alert:nil message:ASLocalizedString(@"是否确定结束游戏？")destructiveAction:^{
            ...
            {
                [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"关闭游戏中，请稍后！")];
            }
        }
    }
    else
    {
        ...
        if (sender.tag != 204) {
            ...
            {
                [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"余额不足")];
                return;
            }
            else
            {
