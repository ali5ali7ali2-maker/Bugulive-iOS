...
- (void)viewDidLoad
{
    ...
    {
      self.provinceStr = self.BuguLive.province;
    }
    if (!self.cityStr.length)
    {
      self.cityStr = self.BuguLive.locationCity;
    }
    if (!self.areaStr.length)
    {
     self.areaStr = self.BuguLive.area;
    }
...
- (void)creatView{
    ...
    if (self.type == 0)
    {
        self.title = ASLocalizedString(@"新增地址");
        [savebutton setTitle:ASLocalizedString(@"保存")forState:UIControlStateNormal];
    }else if (self.type == 1)
    {
        self.title = ASLocalizedString(@"修改地址");
        [savebutton setTitle:ASLocalizedString(@"修改")forState:UIControlStateNormal];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        if (self.BuguLive.area.length > 0)
        {
            if ([self.detailArea hasPrefix:self.BuguLive.area])
            {
                self.detailArea = [self.detailArea stringByReplacingOccurrencesOfString:self.BuguLive.area withString:@""];
            }
        }
        _cell.smallAddressFiled.text = self.detailArea;
    }else
    {
        if ([self.BuguLive.locateName hasPrefix:self.BuguLive.area])
        {
            self.BuguLive.locateName = [self.BuguLive.locateName stringByReplacingOccurrencesOfString:self.BuguLive.area withString:@""];
        }
        _cell.smallAddressFiled.text = self.BuguLive.locateName;
    }
    return _cell;
}
...
- (void)saveEditButton:(UIButton *)btn
{
    ...
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请输入收货人姓名")];
    ...
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请输入收货手机号")];
         btn.userInteractionEnabled = YES;
        return;
    }
    ...
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请输入行政地区信息")];
         btn.userInteractionEnabled = YES;
        return;
    }
    ...
    {
        [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"请输入详细地址")];
        btn.userInteractionEnabled = YES;
        return;
    }
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
     {
         ...
         {
             [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"收货地址保存成功")];
             [self.navigationController popViewControllerAnimated:YES];
         }else
         {
             [[BGHUDHelper sharedInstance] tipMessage:ASLocalizedString(@"收货地址保存不成功")];
         }
