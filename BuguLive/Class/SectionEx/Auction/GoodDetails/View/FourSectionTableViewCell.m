...
- (void)creatCellWithModel:(AcutionHistoryModel *)model withRow:(int)row
{
    ...
    if ([moneyStr integerValue] > 1000)
    {
        moneyStr = [NSString stringWithFormat:ASLocalizedString(@"%.3fK"),[moneyStr floatValue]/1000];
    }else
    {
        if (!moneyStr.length)
        {
    ...
    if (row == 0)
    {
        ...
        self.stateLabel.text = ASLocalizedString(@"领先");
    ...
    }else
    {
        ...
        self.stateLabel.text = ASLocalizedString(@"出局");
