...
- (CGFloat)setCellWithPlace:(NSString *)palceString andPlace:(NSString *)timeString
{
    ...
    {
        palceString = ASLocalizedString(@"约会地点: 福建省 厦门市 万达广场");
    }else
    {
        palceString = [NSString stringWithFormat:ASLocalizedString(@"约会地点: %@"),palceString];
    }
    
    if (timeString.length<1)
    {
        timeString = ASLocalizedString(@"约会时间: 2016年06月06日 18:00");
    }else
    {
        timeString = [NSString stringWithFormat:ASLocalizedString(@"约会时间: %@"),timeString];
    }
