...
{
    [super viewDidLoad];
    self.navigationItem.title = ASLocalizedString(@"选择位置");
    ...
    dispatch_async(queue, ^{
        ...
        }else if (self.BuguLive.latitude && self.BuguLive.longitude){
            CLLocationCoordinate2D pt = (CLLocationCoordinate2D){self.BuguLive.latitude, self.BuguLive.longitude};
            _myMapView.centerCoordinate = pt;
        }
...
- (void)initMyView{
    ...
    [_rightTopBtn setTitle:ASLocalizedString(@"我的位置")forState:UIControlStateNormal];
    ...
    _searchTF.placeholder = ASLocalizedString(@"请输入要查询的地址名称");
    ...
    [_submitBtn setTitle:ASLocalizedString(@"确定")forState:UIControlStateNormal];
...
- (void)btnAction:(id)sender{
    if (sender == _rightTopBtn) {
        if (self.BuguLive.latitude && self.BuguLive.longitude){
            CLLocationCoordinate2D pt = (CLLocationCoordinate2D){self.BuguLive.latitude, self.BuguLive.longitude};
            _myMapView.centerCoordinate = pt;
        }
        ...
        if (_fromType == 0) {
            self.BuguLive.latitude = _myMapView.centerCoordinate.latitude;
            self.BuguLive.longitude = _myMapView.centerCoordinate.longitude;
        }
        [self.navigationController popViewControllerAnimated:YES];
    }
}
...
- (void)mapView:(QMapView *)mapView didUpdateUserLocation:(QUserLocation *)userLocation updatingLocation:(BOOL)updatingLocation{
    ...
    if (coordinate.latitude && coordinate.longitude) {
        self.BuguLive.latitude = coordinate.latitude;
        self.BuguLive.longitude = coordinate.longitude;
    }
    NSLog(@"%f-------%f",self.BuguLive.latitude,self.BuguLive.longitude);
...
- (void)searchWithReverseGeoCodeSearchOption:(QMSReverseGeoCodeSearchOption *)reverseGeoCodeSearchOption didReceiveResult:(QMSReverseGeoCodeSearchResult *)reverseGeoCodeSearchResult
{
    ...
    if (reverseGeoCodeSearchResult)
    {
        if (reverseGeoCodeSearchResult.address && ![reverseGeoCodeSearchResult.address isEqualToString:@""]){
            ...
            self.BuguLive.locationCity = reGeoCodeAdInfo.city;
            self.BuguLive.locateName = reverseGeoCodeSearchResult.formatted_addresses.recommend;
            self.BuguLive.province = reverseGeoCodeSearchResult.ad_info.province;
            self.BuguLive.area = reverseGeoCodeSearchResult.ad_info.district;
            NSMutableDictionary *mdic = [NSMutableDictionary dictionary];

            [mdic setObject:[NSString stringWithFormat:@"%.8f",self.BuguLive.longitude] forKey:@"lng"];
            [mdic setObject:[NSString stringWithFormat:@"%.8f",self.BuguLive.latitude] forKey:@"lat"];
            ...
            if (![reverseGeoCodeSearchResult.ad_info.district isEqual:@""]&&reverseGeoCodeSearchResult.ad_info.district) {
               ...
               self.BuguLive.addressJsonStr = [BGUtils dataTOjsonString:mdic];
            _addressStr = self.BuguLive.locateName;
...
- (void)textChangeAction{
    ...
    if ([_searchTF.text isEqualToString:@""]) {
        ...
        [_searchAddressController searchCityname:self.BuguLive.locationCity keyword:_searchTF.text];
    }
}
