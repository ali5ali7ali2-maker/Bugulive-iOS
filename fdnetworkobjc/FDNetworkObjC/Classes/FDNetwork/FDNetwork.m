...
- (void)GET:(NSString *)URLString param:(NSDictionary *)param success:(void (^)(id _Nonnull))success failure:(void (^)(NSString * _Nonnull))failure{
    ...
    //申明请求的数据是json类型
    manager.requestSerializer = [self requestSerializerWithType:[FDNetwork shareInstance].requestContentType];
    //申明返回的结果是json类型
    manager.responseSerializer = [self responseSerializerWithType:[FDNetwork shareInstance].responseContentType];
    //如果报接受类型不一致请替换一致text/html或别的
    manager.responseSerializer.acceptableContentTypes = [self acceptableContentTypes];
    //超时时间
    manager.requestSerializer.timeoutInterval = [FDNetwork shareInstance].timeoutInterval;
    //证书
...
- (void)POST:(NSString *)URLString param:(NSDictionary *)param success:(void (^)(id _Nonnull))success failure:(void (^)(NSString * _Nonnull))failure{
    ...
    [manager POST:URLString parameters:param headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        FDNetworkResponseModel *model = [FDNetworkResponseModel mj_objectWithKeyValues:responseObject];
        if (model.code.intValue == 1) {
            success(model);
        }else{
